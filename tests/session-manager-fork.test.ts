...
function srcSessionRow(): Record<string, unknown> {
  return {
    ...
    mounted_paths: JSON.stringify([
      { virtual: "/mnt/proj", real: "/tmp/proj" },
    ]),
...
const entries: SessionEntry[] = [
  {
    ...
    message: {
      role: "user",
      content: [{ type: "text", text: "帮我设计登录页" }],
      timestamp: 1,
    },
  },
  {
  ...
  },
  {
    ...
    message: {
      role: "user",
      content: [{ type: "text", text: "改用 OAuth" }],
      timestamp: 3,
    },
  },
  {
...
describe("SessionManager.forkSession (JSONL forkFrom)", () => {
  it("在 msg2（第一条助手消息）分叉：调 forkSessionFile、继承设置、status idle", async () => {
    ...
    expect(fork.mountedPaths).toEqual([
      { virtual: "/mnt/proj", real: "/tmp/proj" },
    ]);
    ...
    expect(db.sessions.update).toHaveBeenCalledWith(fork.id, {
      pi_session_file: "/tmp/forked.jsonl",
    });
  });
  ...
  it("工具结果（tool_result 块）分叉 → 抛错，不创建会话", async () => {
    await expect(
      manager.forkSession("sess-src", "msg-tr", "（分叉）"),
    ).rejects.toThrow("仅支持从助手消息分叉");
  ...
  it("用户消息分叉 → 抛错，不创建会话", async () => {
    await expect(
      manager.forkSession("sess-src", "msg1", "（分叉）"),
    ).rejects.toThrow("仅支持从助手消息分叉");
    expect(db.sessions.create).not.toHaveBeenCalled();
    expect(forkSessionFileMock).not.toHaveBeenCalled();
  });
  ...
  it("forkSessionFile 失败 → 删除新会话记录，无半成品", async () => {
    ...
    await expect(
      manager.forkSession("sess-src", "msg2", "（分叉）"),
  ...
  it("源会话完全不被修改（仅读 + forkFrom）", async () => {
    ...
    expect(
      (db.sessions.update as ReturnType<typeof vi.fn>).mock.calls.length,
  ...
  it("forks a live (uuidv4) message from messageCache", async () => {
    // 模拟实时消息：saveMessage 把 uuidv4 消息 push 进 messageCache
    manager.saveMessage({
      id: "msg1",
      sessionId: "sess-src",
      role: "user",
    ...
    manager.saveMessage({
      ...
      sessionId: "sess-src",
      role: "assistant",
      content: [{ type: "text", text: "实时回复" }],
      timestamp: 2,
    });
    const fork = await manager.forkSession("sess-src", "live-1", "（分叉）");
    expect(fork.id).not.toBe("sess-src");
    // forkIdx=1 → entries 第 2 条 message entry = "msg2"（非 uuidv4 "live-1"）
    expect(forkSessionFileMock.mock.calls[0][3]).toBe("msg2");
  });
});
