...
describe('runDiagnostics TLS step', () => {
  ...
  it('model step uses probeWithAgentSdk', async () => {
    mocks.probeWithAgentSdk.mockResolvedValue({ ok: true, latencyMs: 15 });
    ...
    expect(result.overallOk).toBe(true);
    expect(mocks.probeWithAgentSdk).toHaveBeenCalledWith(
  ...
  it('model step reports failure from probeWithAgentSdk', async () => {
    mocks.probeWithAgentSdk.mockResolvedValue({
      ok: false,
