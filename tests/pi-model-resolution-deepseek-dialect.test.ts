...
async function captureBody(
  ...
  try {
    ...
    for await (const _event of events as never) {
      /* drain */
    }
  } catch {
    /* 上游形状不影响我们对请求体的断言 */
...
describe("cloud deepseek models speak the DeepSeek dialect", () => {
  ...
  it("only fills missing flags instead of clobbering existing ones", () => {
    ...
    const preset = applyPiModelRuntimeOverrides(
      {
        ...
        compat: {
          thinkingFormat: "future-format",
          maxTokensField: "future_field",
    ...
    expect(preset.compat?.requiresReasoningContentOnAssistantMessages).toBe(
      false,
    );
  });
  ...
  it("does not treat a same-prefix host as our endpoint", () => {
    const model = applyPiModelRuntimeOverrides(
      buildSyntheticPiModel(
        ...
        "custom",
        "openai",
        "https://api.deskwand.com.evil.com/api/models",
        undefined,
        undefined,
        undefined,
        undefined,
      ...
      {
        configProvider: "openai",
        customBaseUrl: "https://api.deskwand.com.evil.com/api/models",
        rawProvider: "custom",
        customProtocol: "openai",
      } as never,
    );
  ...
  it("sends system role, max_tokens, no store, and thinking disabled when no effort is given", async () => {
    const body = await captureBody(resolveCloudModel("deepseek-flash"), {
      maxTokens: 16384,
    });
...
describe("cloud deepseek image capability", () => {
  it("keeps the cloud text id text-only even though the upstream catalog now advertises images", () => {
    ...
    // 云端 custom/deepseek-flash 从未验证过接受 image_url，故仍按纯文本处理。
    expect(resolveCloudModel("deepseek-flash").input).toEqual(["text"]);
  });

  it("keeps the registry image capability for the vision id", () => {
    expect(resolveCloudModel("deepseek-v4-flash-vision-exp").input).toEqual([
      "text",
      "image",
    ]);
  });
