...
describe("Main process window/config behavior", () => {
  ...
  it("synchronizes OAuth SDK catalogs before creating the renderer window", () => {
    const source = fs.readFileSync(indexPath, "utf8");
    const bootstrapStart = source.indexOf("app\n  .whenReady()");
    const createWindowPosition = source.indexOf(
      "createWindow();",
      bootstrapStart,
    );
    const bootstrap = source.slice(bootstrapStart, createWindowPosition);
