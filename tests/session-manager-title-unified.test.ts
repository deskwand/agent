...
describe('SessionManager unified title generation', () => {
  const previous = {
    disableOmagtUnified: process.env.COWORK_DISABLE_OMAGT_UNIFIED,
  ...
  beforeEach(() => {
    delete process.env.COWORK_DISABLE_OMAGT_UNIFIED;
  ...
  afterEach(() => {
    if (previous.disableOmagtUnified === undefined) {
      delete process.env.COWORK_DISABLE_OMAGT_UNIFIED;
    } else {
      process.env.COWORK_DISABLE_OMAGT_UNIFIED = previous.disableOmagtUnified;
    }
  ...
  it('routes gemini title generation through Agent SDK even when unified mode flag is disabled', async () => {
    process.env.COWORK_DISABLE_OMAGT_UNIFIED = '1';
