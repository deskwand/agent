...
function Write-Step {
  ...
  Write-Host "[OMAGT Cleanup] $Message"
}
...
function Get-OMARegistryEntries {
  $registryGlobs = @(
  ...
  foreach ($glob in $registryGlobs) {
    ...
    $items = Get-ItemProperty -Path $glob -ErrorAction SilentlyContinue | Where-Object {
      $_.DisplayName -like "OMAGT*" -or $_.Publisher -eq "OMAGT Team"
    }
...
function Stop-OMAProcesses {
  $processes = @(Get-CimInstance Win32_Process -ErrorAction SilentlyContinue | Where-Object {
    $_.Name -ieq "OMAGT.exe" -or $_.ExecutablePath -like "*\OMAGT.exe"
  })

  if ($processes.Count -eq 0) {
    Write-Step "No running OMAGT processes found."
    return
  }
...
$registryEntries = @(Get-OMARegistryEntries)
...
Add-UniquePath -List $installPaths -PathValue (Join-Path $env:LOCALAPPDATA "Programs\OMAGT")

$appDataPaths = [System.Collections.Generic.List[string]]::new()
Add-UniquePath -List $appDataPaths -PathValue (Join-Path $env:APPDATA "OMAGT")
Add-UniquePath -List $appDataPaths -PathValue (Join-Path $env:APPDATA "oma")
Add-UniquePath -List $appDataPaths -PathValue (Join-Path $env:LOCALAPPDATA "OMAGT")
Add-UniquePath -List $appDataPaths -PathValue (Join-Path $env:LOCALAPPDATA "oma")

Write-Host ""
Write-Step "This tool removes broken OMAGT Windows install leftovers."
...
if ($failures.Count -eq 0) {
  Write-Step "Cleanup finished. You can rerun the OMAGT installer now."
