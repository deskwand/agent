...
export class RemoteGateway extends EventEmitter {
  ...
  private authAttempts: Map<string, { count: number; resetTime: number }> =
    new Map();
    ...
    try {
      ...
      await new Promise<void>((resolve, reject) => {
        this.httpServer!.listen(this.config.port, this.config.bind, () => {
          log(
            `[Gateway] HTTP/WS server listening on ${this.config.bind}:${this.config.port}`,
          );
          resolve();
        });
  ...
  registerChannel(channel: IChannel): void {
    if (this.channels.has(channel.type)) {
      logWarn(
        `[Gateway] Channel ${channel.type} already registered, replacing...`,
      );
    }
  ...
  setMessageInterceptor(
    interceptor: (message: RemoteMessage) => Promise<boolean> | boolean,
  ): void {
  ...
  private async sendToChannel(response: RemoteResponse): Promise<void> {
    ...
    try {
      ...
      logError(
        `[Gateway] Failed to send message to channel ${response.channelType}:`,
        error,
      );
    }
  }
  ...
  private handleWebhook(
    ...
    req.on("end", () => {
      ...
      try {
        ...
        if (listenerCount === 0) {
          log(
            `[Gateway] No listeners for webhook:${channelType}, returning OK`,
          );
          res.writeHead(200, { "Content-Type": "application/json" });
  ...
  private async handleWSClientMessage(
    client: WSClient,
    message: WSMessage,
  ): Promise<void> {
    try {
