...
export class RemoteManager extends EventEmitter {
  ...
  private sessionChannelMapping: Map<
    string,
    { channelType: ChannelType; channelId: string }
  > = new Map();
  ...
  private interactionResolvers: Map<string, (response: string) => void> =
    new Map();

  // Response buffer for collecting messages before sending (to avoid spam)
  private responseBuffers: Map<
    string,
    { texts: string[]; lastSent: number; toolSteps: string[] }
  > = new Map();
  ...
  setAgentExecutor(executor: AgentExecutor): void {
    ...
    if (executor.validateWorkingDirectory) {
      this.messageRouter.setWorkingDirectoryValidator(
        executor.validateWorkingDirectory,
      );
    }
  ...
  async start(): Promise<void> {
    ...
    try {
      ...
      if (tunnelUrl) {
        ...
        log(
          "[RemoteManager] Feishu Webhook URL:",
          `${tunnelUrl}/webhook/feishu`,
        );
      }
      ...
      if (currentAuth.mode === "token" || currentAuth.token) {
        ...
        switch (config.dm.policy) {
          ...
          case "allowlist": {
            ...
            const feishuEntries = (config.dm.allowFrom ?? []).map(
              (id) => `feishu:${id}`,
            );
  ...
  async handleQuestionRequest(
    ...
    log(
      "[RemoteManager] Handling question request for remote session:",
      remoteSessionId,
    );
      ...
      setTimeout(
        () => {
          this.withInteractionLock(async () => {
            ...
            logError("[RemoteManager] Question timeout lock error:", err),
          );
        },
        5 * 60 * 1000,
      );
  ...
  async handlePermissionRequest(
    ...
    log(
      "[RemoteManager] Handling permission request for remote session:",
      remoteSessionId,
    );
    ...
    if (config.gateway.autoApproveSafeTools) {
      const safeTools = [
        ...
        "WebFetch",
        "WebSearch",
        // MCP Chrome tools (for external browsing)
    ...
    try {
      ...
      logError(
        "[RemoteManager] Failed to send permission request to channel:",
        err,
      );
      await this.withInteractionLock(async () => {
      ...
      setTimeout(
        () => {
          this.withInteractionLock(async () => {
            ...
            logError("[RemoteManager] Permission timeout lock error:", err),
          );
        },
        5 * 60 * 1000,
      );
  ...
  async handlePotentialInteractionResponse(
    ...
    return this.withInteractionLock(async () => {
      ...
      for (const [id, interaction] of this.pendingInteractions) {
        const channelInfo = this.sessionChannelMapping.get(
          interaction.remoteSessionId,
        );
        ...
        if (
          ...
          if (
            !interaction.ownerSenderId ||
            senderId !== interaction.ownerSenderId
          ) {
            log(
              "[RemoteManager] Ignoring interaction response from non-owner sender:",
              senderId,
            );
            continue;
          }
          ...
          if (resolver) {
            ...
            if (interaction.type === "question") {
              ...
              const response = this.parseQuestionResponse(
                messageText,
  ...
  async sendResponseToChannel(
    ...
    if (!remoteSessionId) {
      log(
        "[RemoteManager] Not a remote session, skipping channel response:",
        actualSessionId,
      );
      return;
    }
    ...
    if (!channelInfo || !this.gateway) {
      logError(
        "[RemoteManager] No channel info for remote session:",
        remoteSessionId,
      );
      return;
    }
    ...
    if (!this.responseBuffers.has(actualSessionId)) {
      this.responseBuffers.set(actualSessionId, {
        texts: [],
        lastSent: 0,
        toolSteps: [],
      });
  ...
  async sendToolProgress(
    ...
    if (status === "running") {
      ...
      this.responseBuffers
        .get(actualSessionId)!
        .toolSteps.push(`${emoji} ${statusText}`);
      return;
    }
  ...
  private async executeAgent(
    ...
    if (isNewSession) {
      ...
      log(
        "[RemoteManager] Session mapping stored:",
        newSession.id,
        "<->",
        sessionId,
      );
      log(
        "[RemoteManager] Emitting session update to renderer for:",
        newSession.id,
      );
      ...
      if (!actualSessionId) {
        throw new Error(
          `No actual session ID found for remote session: ${sessionId}`,
        );
      }
      log(
        "[RemoteManager] Continuing session:",
        actualSessionId,
        "for remote:",
        sessionId,
      );
