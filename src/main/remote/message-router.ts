...
export class MessageRouter {
  ...
  private async processQueue(sessionId: string): Promise<void> {
    ...
    if (this.processingSession.has(sessionId)) {
      log(
        "[MessageRouter] Session already processing, will process later:",
        sessionId,
      );
      return;
    }
  ...
  private async processMessage(
    ...
    const baseWorkingDirectory =
      mapping?.workingDirectory || this.defaultWorkingDirectory;
    ...
    if (cwd && !resolvedCwd) {
      await this.sendErrorResponse(
        message,
        new Error(
          "Relative working directory requires an existing base directory",
        ),
      );
      return;
    }
    ...
    if (!prompt && resolvedCwd) {
      const ensuredMapping = this.ensureSessionMapping(
        message,
        sessionKey,
        sessionId,
      );
    ...
    log(
      "[MessageRouter] Using working directory:",
   ...
   */
  private extractPromptAndCwd(message: RemoteMessage): {
    prompt: string;
    cwd?: string;
  } {
