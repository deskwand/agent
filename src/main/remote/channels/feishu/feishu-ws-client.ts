...
export class FeishuWSClient extends EventEmitter {
  ...
  async replyMessage(
    ...
    try {
      ...
      log("[FeishuWS] Replying to message:", {
        msgType,
        contentLength: messageContent.length,
      });
  ...
  private async handleMessageReceive(
    ...
    const senderId = senderData?.sender_id as
  ...
  private scheduleReconnect(): void {
    ...
    log(
      `[FeishuWS] Scheduling reconnect in ${delay}ms (attempt ${this.reconnectAttempts})`,
    );
