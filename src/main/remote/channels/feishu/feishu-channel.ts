...
export class FeishuChannel extends ChannelBase {
  ...
  handleWebhook(
    ...
    if (!signature) {
      logWarn(
        "[Feishu] Webhook request rejected: missing X-Lark-Signature header",
      );
    ...
    try {
      ...
      if (this.config.encryptKey && data.encrypt) {
        log(
          "[Feishu] Encrypted message received, decryption not yet implemented",
        );
        // TODO: Implement message decryption
        return {
          status: 501,
          data: { code: 1, msg: "Encrypted webhook not yet supported" },
        };
      }
   ...
   */
  private stripBotMentionFromText(
    text: string,
  ...
  private parseMessageContent(
    ...
    try {
      const contentJson = JSON.parse(message.content as string) as Record<
        string,
        unknown
      >;
  ...
  private async sendMessage(
    ...
    if (this.config.useWebSocket && this.wsClient) {
      ...
      if (msgType === "interactive") {
        ...
        if (replyTo) {
          await this.wsClient.replyMessage(
            replyTo,
        ...
        // For text messages
        const textContent =
          (typeof msgContent.text === "string" ? msgContent.text : undefined) ||
          content.text ||
          "";
  ...
  private async uploadImage(image: {
    ...
    if (image.url) {
      ...
      const isPrivateIp = (ip: string): boolean =>
        /^(127\.|10\.|172\.(1[6-9]|2[0-9]|3[01])\.|192\.168\.|169\.254\.|0\.)/.test(
          ip,
        ) ||
        /^::1$/.test(ip) ||
        /^[fF][cCdD][0-9a-fA-F]{2}:/.test(ip) ||
        // IPv4-mapped IPv6 addresses (::ffff:192.168.x.x etc.)
        /^::ffff:(127\.|10\.|172\.(1[6-9]|2[0-9]|3[01])\.|192\.168\.|169\.254\.|0\.)/.test(
          ip,
        );
