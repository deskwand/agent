...
export async function maybeGenerateSessionTitle(
  ...
  if (deps.shouldAbort?.()) {
    deps.log(
      "[SessionTitle] Skip: title flow aborted before start",
      deps.sessionId,
    );
    return;
  }
  ...
  try {
    generatedTitle = normalizeGeneratedTitle(
      await deps.generateTitle(titlePrompt),
    );
  } catch (error) {
  ...
  if (deps.shouldAbort?.()) {
    deps.log(
      "[SessionTitle] Skip: title flow aborted after generation",
      deps.sessionId,
    );
    return;
  }
  ...
  if (
    ...
    latestTitle !== defaultTitle
  ) {
    deps.log(
      "[SessionTitle] Skip: title changed before update",
      deps.sessionId,
    );
    return;
  }
  ...
  if (updated) {
    ...
    deps.log(
      "[SessionTitle] Title update no-op (session may have been deleted)",
      deps.sessionId,
    );
  }
}
