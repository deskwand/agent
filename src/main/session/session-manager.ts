...
export class SessionManager {
  ...
  private promptQueues: Map<
    ...
    Array<{
      ...
      turnId: string;
      autoGenerated?: boolean;
      /** 元素引用展示投影：只为让 runner 写进 SDK details，不用于实时气泡 */
  ...
  private createAgentRunner(): void {
    this.agentRunner = new AgentRunner(
      {
        ...
        onBackgroundAgentComplete: (sessionId: string, agentId: string) => {
          ...
          if (!session) {
            logError(
              "[SessionManager] auto-continue: session not found:",
              sessionId,
            );
            return;
          }
          log(
            "[SessionManager] auto-continue: enqueued for",
            sessionId,
            agentId,
          );
          // 与 goal 续跑（resumeGoalPrompts）同形，标 autoGenerated：这条注入
          // 消息会（以隐藏形式）发给渲染层，渲染层才能为这一轮建出
          // pendingTurn → activeTurn。走 continueSession 会把 autoGenerated
          // 写死成 undefined —— 于是整轮没有 activeTurn：状态栏空白、正在跑的
          // 工具卡看起来像已完成、/compact 闸门失效。
          this.enqueuePrompt(session, prompt, undefined, undefined, true);
        },
      },
      this.pathResolver,
      this.mcpManager,
    ...
    // 注入 entriesReader：让 SessionManager 的 getMessages/getMessagesPage
    // 能从 agentRunner 常驻 cache / pi_session_file 读 JSONL entries。
    this.setEntriesReader(
      (sessionId) => this.agentRunner.getSessionEntries?.(sessionId) ?? null,
    );
  ...
  private async processPrompt(
    ...
    return runWithLogContext({ sessionId: session.id, traceId }, async () => {
      ...
      try {
        ...
        const firstFileAttachment = messageContent.find(
          (c): c is FileAttachmentContent => c.type === "file_attachment",
        );
        firstAttachmentName = firstFileAttachment?.filename ?? null;
        ...
        if (fileAttachments.length > 0) {
          ...
          if (imageFiles.length > 0) {
            const imageInfo = imageFiles
              ...
              .join(", ");
            parts.push(
              `[Image attachment(s) included in this message: ${imageInfo}]`,
            );
          }
          if (parts.length > 0) {
        ...
        existingMessages = this.getMessages(session.id);
        const userMessage: Message = {
          ...
          // 自动消息不复用用户附件（与 autoGenerated 语义一致）：
          // 目标/调度产生的消息不该带上某次手工拾取的引用。
        ...
        // 标题只依赖用户输入，不依赖本轮助手回答是否成功：首轮报错也要起名。
        // 仍然串行在对话结束之后发起，避免与主请求竞争同一上游配额/通道。
        if (userMessageSaved) {
          const userMessageCount =
            existingMessages.filter((message) => message.role === "user")
              .length + 1;
          void this.runSessionTitleGeneration(
            session,
            prompt,
            userMessageCount,
            firstAttachmentName,
    ...
    try {
      await maybeGenerateSessionTitle({
        ...
        generateTitle: async (titlePrompt) => {
          ...
          const candidates = await this.resolveTitleModelCandidates(session);
  ...
  /**
   * 启动时清掉 DB 里残留的 running：上个进程被强杀时没人给这些会话收尾，留着会
   * 让渲染层以为它们还在跑（停止按钮常亮、状态栏一直「思考中…」）。
   * 必须在 recoverGoals() 之前调用 —— 那个会把 goal 会话真的跑起来。
   */
  resetStaleRunningStatuses(): void {
    let cleared = 0;
    for (const row of this.db.sessions.getAll()) {
      if (row.status !== "running") continue;
      // 不传 updated_at：db.sessions.update 自己会写（列表按它倒序）。副作用是被
      // 强杀的那批会话会被顶到列表顶部 —— 它们本来就是最后活跃的会话，可接受。
      this.db.sessions.update(row.id, { status: "idle" });
      cleared += 1;
    }
    if (cleared > 0) {
      log("[SessionManager] Stale running sessions reset to idle:", cleared);
    }
  }
  ...
  // Cache message in memory (JSONL single-write; no DB write)
  saveMessage(message: Message): void {
    // JSONL 单写后：saveMessage 只维护内存 cache（流式 UI 用），不再写 SQLite。
    ...
    log("[SessionManager] Message cached:", message.id, "role:", message.role);
  }
  ...
  getMessages(sessionId: string): Message[] {
    ...
    const fromEntries = this.readMessagesFromEntries(sessionId);
  ...
  getMessagesPage(
    ...
    if (fromEntries) {
      return (
        sliceCachedPage(fromEntries, beforeId, limit) ?? {
          messages: [],
          hasMore: false,
        }
      );
  ...
  private entriesReader: ((sessionId: string) => SessionEntry[] | null) | null =
    null;

  /**
   * 由 agentRunner 注入：读某会话的 pi JSONL entries（无 JSONL 返回 null）。
   * JSONL 单写后成为消息唯一真相源；无 entries（老会话未回填）时返回空历史。
   */
  setEntriesReader(reader: (sessionId: string) => SessionEntry[] | null): void {
    this.entriesReader = reader;
  }

  private readMessagesFromEntries(sessionId: string): Message[] | null {
    if (!this.entriesReader) return null;
    const entries = this.entriesReader(sessionId);
  ...
  /**
   * 标题生成的模型候选：轻量任务模型优先，会话自己的模型兜底。
   * 两者解析到同一通道时去重，避免同一个坏通道被试两遍。
   */
  private async resolveTitleModelCandidates(
    session: Session,
  ): Promise<Array<{ label: string; config: AppConfig }>> {
    const appConfig = configStore.getAll();
    const resolved = resolveUtilityModelConfig(appConfig, appConfig.model);
    const candidates: Array<{ label: string; config: AppConfig }> = [
      { label: "utility", config: buildUtilityAppConfig(appConfig, resolved) },
    ];

    try {
      const sessionRuntime = await modelResolutionService.resolve({
        sessionProviderProfileKey: session.providerProfileKey,
        sessionModel: session.model,
        appConfig,
      });
      const sessionResolved: ResolvedUtilityModelConfig = {
        provider: sessionRuntime.providerType,
        customProtocol: sessionRuntime.customProtocol,
        apiKey: sessionRuntime.apiKey ?? "",
        baseUrl: sessionRuntime.baseUrl,
        model: sessionRuntime.modelId,
        timeoutMs: resolved.timeoutMs,
        activeProviderKey: sessionRuntime.providerProfileKey,
      };
      // 去重键用 profile key 而非 baseUrl：utility 侧的 baseUrl 是原始存储值，
      // session 侧经 normalizeOpenAICompatibleBaseUrl 处理过，直接比对会出现
      // 同一通道判为两条；带上 profile key 则不会把两个不同凭证的 profile 折叠。
      const channelKey = (candidate: ResolvedUtilityModelConfig) =>
        `${candidate.activeProviderKey}|${candidate.provider}|${candidate.model}`;
      if (channelKey(resolved) !== channelKey(sessionResolved)) {
        candidates.push({
          label: "session",
          config: buildUtilityAppConfig(appConfig, sessionResolved),
        });
      }
    } catch (error) {
      logWarn("[SessionTitle] Session model candidate unavailable:", error);
    }
