...
export class WSLBridge implements SandboxExecutor {
  ...
  static async checkWSLStatus(): Promise<WSLStatus> {
    try {
      ...
      if (!distroWorks) {
        log(
          "[WSL] Selected distro is not responding. WSL service may need restart.",
        );
        log(
          "[WSL] Try: wsl --shutdown (in PowerShell as Admin), then restart the app",
        );
        return { available: false };
      }
  ...
  static async testDistro(distro: string): Promise<boolean> {
    try {
      ...
      if (errMsg.includes("E_UNEXPECTED") || errMsg.includes("4294967295")) {
        log(
          "[WSL] Distro test failed - WSL service error. Try: wsl --shutdown",
        );
      }
      return false;
    }
  ...
  static async installNodeInWSL(distro: string): Promise<boolean> {
    ...
    if (!distroWorks) {
      logError(
        "[WSL] Cannot execute commands in WSL. WSL service may need restart.",
      );
    ...
    try {
      ...
      log(
        "[WSL] nvm installation failed, trying apt...",
    ...
    try {
      ...
      await execFileAsync("wsl", ["-d", distro, "-e", "sudo", "-n", "true"], {
        timeout: 5000,
      });
  ...
  static async installPipInWSL(distro: string): Promise<boolean> {
    ...
    try {
      ...
      try {
        ...
        log(
          "[WSL] Passwordless sudo available, installing python3-pip via apt...",
        );
        await execFileAsync(
          "wsl",
          [
  ...
  private async _initialize(config: SandboxConfig): Promise<void> {
    ...
    if (!status.pythonAvailable) {
      ...
      if (!installed) {
        log(
          "[WSL] Failed to install Python in WSL (non-critical, continuing...)",
        );
      }
  ...
  private async startAgent(): Promise<void> {
    ...
    if (
      ...
      throw new Error(
        `Agent path does not contain expected path segment: ${agentPath}`,
      );
    }
    ...
    this.wslProcess = spawn(
      "wsl",
      ["-d", this.distro, "--", "bash", "-c", nodeCommand],
      {
  ...
  async listDirectory(dirPath: string): Promise<DirectoryEntry[]> {
    ...
    const result = await this.sendRequest<{ entries: DirectoryEntry[] }>(
      "listDirectory",
      {
        path: wslPath,
      },
