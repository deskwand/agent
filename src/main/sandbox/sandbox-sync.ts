...
export class SandboxSync {
  ...
  static async initSync(
    ...
    if (existingSession && existingSession.initialized) {
      ...
      try {
        ...
        log(
          `[SandboxSync] Sandbox directory no longer exists, reinitializing...`,
        );
    ...
    try {
      ...
      await this.wslExec(
        distro,
        `mkdir -p '${this.shellEscapePath(sandboxPath)}'`,
      );
      ...
      log(
        `[SandboxSync] Sync complete: ${fileCount} files, ${this.formatSize(totalSize)}`,
      );
  ...
  static async cleanup(sessionId: string): Promise<void> {
    ...
    try {
      ...
      const sandboxRoot = session.sandboxPath.substring(
        0,
        session.sandboxPath.lastIndexOf("/"),
      );
      ...
      await this.wslExec(
        session.distro,
        `rm -rf '${this.shellEscapePath(session.sandboxPath)}'`,
      );
      sessions.delete(sessionId);
  ...
  static async syncFileToSandbox(
    ...
    log(
      `[SandboxSync] Syncing file to sandbox: ${windowsSourcePath} -> ${sandboxDestPath}`,
    );
    ...
    try {
      ...
      const destDir = sandboxDestPath.substring(
        0,
        sandboxDestPath.lastIndexOf("/"),
      );
      await this.wslExec(
        session.distro,
        `mkdir -p '${this.shellEscapePath(destDir)}'`,
      );
  ...
  static async cleanupAllSessions(): Promise<void> {
    ...
    const results = await Promise.allSettled(
      sessionIds.map(async (sessionId) => {
        try {
          ...
          logError(
            `[SandboxSync] Failed to cleanup session ${sessionId}:`,
            error,
          );
    ...
    log(
      `[SandboxSync] Cleanup complete: ${succeeded} succeeded, ${failed} failed`,
    );
  }
  ...
  static windowsToSandboxPath(
    ...
    // Normalize paths
    const normalizedWindows = session.windowsPath
      .replace(/\\/g, "/")
      .toLowerCase();
