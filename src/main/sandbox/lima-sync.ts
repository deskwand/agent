...
export class LimaSync {
  ...
  static async initSync(
    ...
    if (sessions.has(sessionId)) {
      ...
      try {
        ...
        await this.limaExec(
          `test -d '${LimaSync.shellEscapePath(existingSession.sandboxPath)}'`,
        );
        return {
          success: true,
    ...
    try {
      ...
      log(
        `[LimaSync] Sync complete: ${fileCount} files, ${this.formatSize(totalSize)}`,
      );
  ...
  static async cleanup(sessionId: string): Promise<void> {
    ...
    try {
      ...
      await this.limaExec(
        `rm -rf '${LimaSync.shellEscapePath(session.sandboxPath)}'`,
      );
  ...
  static async syncFileToSandbox(
    ...
    log(
      `[LimaSync] Syncing file to sandbox: ${macSourcePath} -> ${sandboxDestPath}`,
    );
  ...
  static async copyFileToSandbox(
    ...
    const result = await this.syncFileToSandbox(
      sessionId,
      macPath,
      relativePath,
    );
  ...
  static async cleanupAllSessions(): Promise<void> {
    ...
    log(
      `[LimaSync] Cleanup complete: ${succeeded} succeeded, ${failed} failed`,
    );
  }
