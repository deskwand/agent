...
export class PathGuard {
  ...
  static isPathAllowed(path: string, sessionId: string): ValidationResult {
    ...
    if (normalizedPath.startsWith("/root/.nvm/")) {
      ...
      if (
        nvmReadOnlyPrefixes.some((prefix) => normalizedPath.startsWith(prefix))
      ) {
        return { allowed: true };
      }
      return {
  ...
  static validateCommand(command: string, sessionId: string): ValidationResult {
    ...
    for (const pattern of DANGEROUS_COMMAND_PATTERNS) {
      if (pattern.test(command)) {
        logError(
          `[PathGuard] Blocked dangerous command: ${command.substring(0, 100)}`,
        );
        return {
          allowed: false,
    ...
    if (/\/mnt\/[a-z]/i.test(command)) {
      logError(
        `[PathGuard] Blocked /mnt/ access in command: ${command.substring(0, 100)}`,
      );
      return {
        allowed: false,
  ...
  static convertPathInCommand(
    ...
    const normalizedWorkspace = windowsWorkspacePath
      .replace(/\\/g, "/")
      .toLowerCase();
    ...
    const convertWindowsPath = (
      ...
      if (isPathWithinRoot(normalizedFullPath, normalizedWorkspace, true)) {
        ...
        const relativePath = originalFullPath.substring(
          normalizedWorkspace.length,
        );
