...
export class SandboxAdapter implements SandboxExecutor {
  ...
  private async _initialize(config: SandboxAdapterConfig): Promise<void> {
    ...
    if (sandboxEnabled === false) {
      log(
        "[SandboxAdapter] Sandbox disabled by user configuration, using native mode",
      );
  ...
  private async initializeWSL(config: SandboxAdapterConfig): Promise<void> {
    ...
    log(
      ...
      wslStatus.nodeAvailable
        ? `[OK] ${wslStatus.version || "available"}`
        : "[X] not found",
    );
    log(
      "[SandboxAdapter]   Python:",
      wslStatus.pythonAvailable
        ? `[OK] ${wslStatus.pythonVersion || "available"}`
        : "[X] not found",
    );
    log(
    ...
    if (!wslStatus.nodeAvailable) {
      if (!config.skipInstallPrompts) {
        const shouldInstall = await this.showNodeInstallPrompt(
          config,
          wslStatus.distro!,
        );
        if (!shouldInstall) {
  ...
  private async initializeLima(config: SandboxAdapterConfig): Promise<void> {
    ...
    log(
      ...
      limaStatus.instanceExists ? "[OK]" : "[X] not created",
    );
    log(
      "[SandboxAdapter]   Running:",
      limaStatus.instanceRunning ? "[OK]" : "[X] not running",
    );
    log(
      "[SandboxAdapter]   Node.js:",
      limaStatus.nodeAvailable
        ? `[OK] ${limaStatus.version || "available"}`
        : "[X] not found",
    );
    log(
      "[SandboxAdapter]   Python:",
  ...
  private async initializeNative(config: SandboxAdapterConfig): Promise<void> {
    ...
    if (isWindows) {
      log(
        "[SandboxAdapter] [!] SANDBOX MODE: Native (Windows - No WSL Isolation)",
      );
    } else if (isMac) {
  ...
  private async showWSLNotAvailableWarning(
    ...
    await dialog.showMessageBox(config.mainWindow, {
      ...
      message:
        "Windows Subsystem for Linux (WSL2) is not installed on this system.",
      detail:
  ...
  private async showInstallFailedWarning(
    ...
    if (!config.mainWindow) {
      logWarn(
        `[SandboxAdapter] ${packageName} installation failed, no window to show dialog`,
      );
      return;
    }
  ...
  private async showNativeFallbackWarning(
    config: SandboxAdapterConfig,
  ): Promise<void> {
    if (!config.mainWindow) {
      logWarn(
        "[SandboxAdapter] Falling back to native execution, no window to show dialog",
      );
      return;
    }
  ...
  private async showLimaNotAvailableWarning(
    config: SandboxAdapterConfig,
  ): Promise<void> {
    if (!config.mainWindow) {
  ...
  async reinitialize(config?: SandboxAdapterConfig): Promise<void> {
    ...
    try {
      ...
      const initConfig = config ||
        this._config || { workspacePath: this.state.workspacePath };
