...
const isLimaShellConnectionError = (error: unknown): boolean => {
  ...
  return (
    text.includes("Connection refused") || text.includes("ssh: connect to host")
  );
};
...
export class LimaBridge implements SandboxExecutor {
  ...
  static async checkLimaStatus(): Promise<LimaStatus> {
    try {
      ...
      try {
        ...
        try {
          await execAsync(`limactl info ${LIMA_INSTANCE_NAME}`, {
            timeout: 5000,
          });
          instanceExists = true;
  ...
  static async startLimaInstance(): Promise<boolean> {
    ...
    log(
      "[Lima] This may take several minutes (first start includes image download)...",
    );
  ...
  private async _initialize(config: SandboxConfig): Promise<void> {
    ...
    try {
      ...
      if (
        ...
        cachedStatus.instanceRunning
      ) {
        log("[Lima] Using cached status from bootstrap");
        status = cachedStatus;
      } else {
        log(
          "[Lima] No cached status or instance not running, checking Lima...",
        );
  ...
  private async startAgent(): Promise<void> {
    ...
    if (!hasExpectedSegment) {
      throw new Error(
        `Agent path does not contain expected segments: ${agentPath}`,
      );
    }
