...
import {
  ...
  PROMPT_ITERATION_SYSTEM_PROMPT,
  type MemoryPromptSet,
} from "./memory-prompts";
...
export class MemoryPromptOptimizer {
  ...
  async optimize(options: {
    ...
    for (let round = 1; round <= options.rounds; round += 1) {
      const candidatePrompts = await this.generateCandidate(
        bestPrompts,
  ...
  private async generateCandidate(
    ...
    try {
      ...
      const candidate =
        Array.isArray(candidateContainer.candidates) &&
        candidateContainer.candidates.length > 0
      ...
      if (typeof candidate.coreMemoryUpdateSystemPrompt === "string") {
        next.coreMemoryUpdateSystemPrompt =
          candidate.coreMemoryUpdateSystemPrompt.trim();
      }
      if (typeof candidate.sessionChunkExtractionPrompt === "string") {
        next.sessionChunkExtractionPrompt =
          candidate.sessionChunkExtractionPrompt.trim();
      }
