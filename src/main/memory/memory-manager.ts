...
export class MemoryManager {
  ...
  getMessageHistory(sessionId: string, limit?: number): Message[] {
    ...
    return rows.map((row) => {
      ...
      try {
        ...
        content = [
          { type: "text", text: row.content as string } as ContentBlock,
        ];
      }
  ...
  /**
   * Generate a summary of messages (placeholder - would call Agent API)
   */
  private generateSummary(messages: Message[]): string {
    // In production, this would call Agent API to generate a proper summary
  ...
  searchMemory(sessionId: string, query: string): MemoryEntry[] {
    ...
    const rows = stmt.all(sessionId, `%${escapedQuery}%`) as Record<
      string,
      unknown
    >[];
