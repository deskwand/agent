...
function normalizeChunk(item: Record<string, unknown>): ChunkMemoryItem | null {
  ...
  return {
    ...
    sourceWorkspaceLabel:
      typeof item.source_workspace_label === "string"
        ? item.source_workspace_label
        : undefined,
    sourceSessionId:
      typeof item.source_session_id === "string"
        ? item.source_session_id
        : sessionId,
    sourceSessionTitle:
      typeof item.source_session_title === "string"
        ? item.source_session_title
        : undefined,
    sourceSessionDate:
      typeof item.source_session_date === "string"
    ...
    sourceTurns: Array.isArray(item.source_turns)
      ? item.source_turns.filter(
          (turn): turn is number => typeof turn === "number",
        )
      : [],
...
export class ExperienceMemoryStore {
  ...
  retrieveProgressive(
    ...
    const broadSummaries = [
      ...rankedChunks.map(
        (
          ...
          score: number;
          payload: ProgressiveRetrievalResult["broadSummaries"][number];
        } => ({
          score: item.score,
          payload: {
            type: "chunk",
    ...
    const sessionContexts: SessionContextItem[] = rankedSessions.map(
      (item) => ({
        sessionId: item.record.sessionId,
        sourceWorkspace: item.record.sourceWorkspace,
        sourceSessionTitle: item.record.sourceSessionTitle,
    ...
    const rawSessionCandidates: RawSessionCandidateItem[] = rankedSessions.map(
      (item) => ({
        sessionId: item.record.sessionId,
        sourceWorkspace: item.record.sourceWorkspace,
        sourceSessionTitle: item.record.sourceSessionTitle,
