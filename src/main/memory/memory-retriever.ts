...
export class MemoryRetriever {
  ...
  search(params: MemorySearchParams): MemorySearchResult[] {
    ...
    const defaultWorkspace = normalizeWorkspaceKey(
      params.workspaceKey ?? params.cwd ?? null,
    );
    const explicitSourceWorkspace =
      params.sourceWorkspace !== undefined
        ? normalizeWorkspaceKey(params.sourceWorkspace)
        : null;
  ...
  read(id: string): MemoryReadResult | null {
    ...
    const rawText = session.rawSession
    ...
    return {
      ...
      recordId: parsed.recordId,
      kind:
        parsed.kind === "raw_session" ? "raw_session" : "experience_session",
      title:
  ...
  private readCore(combinedKey: string): MemoryReadResult | null {
    const entry = this.deps
      .getCoreEntries()
      .find((item) => item.combinedKey === combinedKey);
    if (!entry) {
      return null;
    }
    return {
