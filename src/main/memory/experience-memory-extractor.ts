  ...
  async extractSession(params: {
    ...
    return {
      ...
      sessionKeywords: Array.isArray(record.session_keywords)
        ? record.session_keywords.filter(
            (item): item is string => typeof item === "string",
          )
        : Array.isArray(record.sessionKeywords)
          ? record.sessionKeywords.filter(
              (item): item is string => typeof item === "string",
            )
          : [],
      chunks: rawChunks
        .map((item) => {
          ...
          const sourceTurns = Array.isArray(chunk.source_turns)
            ? chunk.source_turns.filter(
                (value): value is number => typeof value === "number",
              )
            : Array.isArray(chunk.sourceTurns)
              ? chunk.sourceTurns.filter(
                  (value): value is number => typeof value === "number",
                )
              : [];
          return {
            summary:
              typeof chunk.summary === "string" ? chunk.summary.trim() : "",
            details:
        ...
        .filter((item): item is ExperienceSessionExtract["chunks"][number] =>
