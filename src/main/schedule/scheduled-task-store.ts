...
function mapTaskUpdatesToRow(
  ...
  if (updates.scheduleConfig !== undefined) {
    mapped.schedule_config = updates.scheduleConfig
      ? JSON.stringify(updates.scheduleConfig)
      : null;
  }
  if (updates.repeatEvery !== undefined)
    mapped.repeat_every = updates.repeatEvery;
  ...
  if (updates.lastRunSessionId !== undefined)
    mapped.last_run_session_id = updates.lastRunSessionId;
