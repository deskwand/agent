...
export class ScheduledTaskManager {
  ...
  update(id: string, updates: ScheduledTaskUpdateInput): ScheduledTask | null {
    ...
    const nextPrompt =
      updates.prompt === undefined ? current.prompt : updates.prompt.trim();
    const nextTitle =
    ...
    if (
      !usesScheduleConfig &&
      nextRepeatEvery !== undefined &&
      nextRepeatEvery === null
    ) {
      nextRepeatUnit = null;
    }
  ...
  toggle(id: string, enabled: boolean): ScheduledTask | null {
    ...
    if (enabled && !isRepeatingTask(current)) {
      ...
      if (oneTimeRunAt <= this.now()) {
        throw new Error(
          "Cannot enable: one-time task is overdue. Edit the schedule first.",
        );
      }
    }
  ...
  private handleTrigger(taskId: string): void {
    ...
    this.executeAndRecord(taskToExecute)
      .catch((err) => {
        ...
        logError(
          `[ScheduledTask] Unhandled error executing task ${taskToExecute.id}:`,
          err,
        );
      })
      .finally(() => {
...
function isRepeatingTask(task: ScheduledTask): boolean {
  return (
    task.scheduleConfig !== null || Boolean(task.repeatEvery && task.repeatUnit)
  );
}
...
function normalizeWeekdays(days: number[]): ScheduledTaskWeekday[] {
  ...
  const normalized = Array.from(
    new Set(
      days.filter(
        (day): day is ScheduledTaskWeekday =>
...
function findNextScheduledSlot(
  ...
  const allowedWeekdays = weekdays
    ? new Set(normalizeWeekdays(weekdays))
    : null;
  ...
  for (let dayOffset = 0; dayOffset <= 14; dayOffset += 1) {
    ...
    if (
      allowedWeekdays &&
      !allowedWeekdays.has(candidateDate.getDay() as ScheduledTaskWeekday)
    ) {
      continue;
    }
