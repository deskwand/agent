...
async function inferMostRecentAppNameFromDisk(): Promise<string | null> {
  try {
    ...
    for (const entry of entries) {
      ...
      const clickHistoryPath = path.join(
        GUI_APPS_DIR,
        entry.name,
        "click_history.json",
      );
...
async function restoreLastAppContext(): Promise<boolean> {
  ...
  try {
    ...
    writeMCPLog(
      `[App Context] Restoring last app context: "${appName}"`,
      "App Init",
    );
...
function getAliasTokensForAppName(appName: string): string[] {
  ...
  for (const group of APP_NAME_ALIAS_GROUPS) {
    ...
    const compactGroup = group.map((token) => compactText(token));
    const matched =
      normalizedGroup.includes(normalizedName) ||
      compactGroup.includes(compactName);
    if (!matched) continue;
...
function scoreDockItemAgainstDescription(
  itemName: string,
  description: string,
): number {
...
function isLikelyAppLaunchVerification(question: string): boolean {
  ...
  const mentionsMenuLike =
    /(menu|菜单|弹窗|popup|面板|widget|小组件|下拉)/i.test(normalized);
...
async function getAllVisitedApps(): Promise<string[]> {
  try {
    ...
    for (const entry of entries) {
      if (entry.isDirectory()) {
        try {
          ...
          writeMCPLog(
            `[getAllVisitedApps] Found app: ${entry.name}`,
            "App List",
          );
        } catch (error) {
    ...
    writeMCPLog(
      `[getAllVisitedApps] Found ${actualAppNames.length} visited apps`,
      "App List",
    );
...
async function loadClickHistoryForApp(appName: string): Promise<void> {
  try {
    ...
    try {
      ...
      if (
        !appHistory ||
        typeof appHistory !== "object" ||
        !Array.isArray(appHistory.clicks)
      ) {
        writeMCPLog(
      ...
      for (const storedClick of appHistory.clicks || []) {
        ...
        const display = config.displays.find(
          (d) => d.index === storedClick.displayIndex,
        );
        if (!display) {
          writeMCPLog(
        ...
        const y = Math.round(
          (storedClick.y_normalized / 1000) * display.height,
        );
...
async function saveLatestClickToHistory(
  ...
  if (!currentAppName) {
    writeMCPLog(
      "[ClickHistory] No app initialized, skipping save",
      "Click History Save",
    );
    return;
  }
  ...
  try {
    ...
    const config = await getDisplayConfiguration();
    const display = config.displays.find(
      (d) => d.index === latestClick.displayIndex,
    );
    ...
    if (existingClickIndex !== -1) {
      ...
      existingHistory.clicks[existingClickIndex].timestamp =
        latestClick.timestamp;
      existingHistory.clicks[existingClickIndex].operation =
        latestClick.operation;
      existingHistory.clicks[existingClickIndex].successCount =
        latestClick.successCount || 0;
...
async function addClickToHistory(
  ...
  const existingEntry = clickHistory.find(
    (entry) =>
      entry.x === x && entry.y === y && entry.displayIndex === displayIndex,
  );
...
async function clearClickHistory(): Promise<void> {
  ...
  if (currentAppName) {
    try {
      ...
      writeMCPLog(
        `[ClickHistory] Deleted click history file: ${filePath}`,
        "Click History",
      );
    } catch (error: unknown) {
      if (
...
async function convertCliclickToCocoaCoordinates(
  ...
  const config = await getDisplayConfiguration();
  const mainDisplay =
    config.displays.find((d) => d.isMain) || config.displays[0];
  ...
  const originYCocoa =
    mainHeight - targetDisplay.height - targetDisplay.originY;
...
async function resolvePythonExec(): Promise<PythonExec | null> {
  ...
  writeMCPLog(
    "[resolvePythonExec] Resolving Python executable...",
    "Python Resolve",
  );
  ...
  writeMCPLog(
    `[resolvePythonExec] Dev environment: ${isDev}`,
    "Python Resolve",
  );
  if (isDev) {
    writeMCPLog(
  ...
  if (envPython && (await pathExists(envPython))) {
    writeMCPLog(
      `[resolvePythonExec] Found explicit override: ${envPython}`,
      "Python Resolve",
    );
  ...
  if (isDev) {
    ...
    try {
      ...
      const { stdout } = await executeCommandSafe(whichCmd, [pythonArg], {
        timeout: 2000,
      });
    ...
    try {
      const testResult = await executeCommandSafe(python3Cmd, ["--version"], {
        timeout: 2000,
      });
      writeMCPLog(
      ...
      if (testResult.stdout || testResult.stderr) {
        ...
        cachedPythonExec = {
          python: pythonPath,
          pythonRoot:
            pythonPath !== python3Cmd
              ? path.resolve(pythonPath, "..", "..")
              : "",
          env: {
            ...baseEnv, // Keep all current environment variables (including conda settings)
            // Don't set PYTHONHOME in dev mode to preserve conda/venv environment
  ...
  if (PLATFORM === "darwin") {
    const arch = process.arch === "arm64" ? "arm64" : "x64";
    writeMCPLog(
      `[resolvePythonExec] Checking bundled Python (arch: ${arch})`,
      "Python Resolve",
    );
    const packaged = await resolveBundledExecutable(
    ...
    if (pythonPath) {
      ...
      writeMCPLog(
        `[resolvePythonExec] Using bundled Python: ${pythonPath}`,
        "Python Resolve",
      );
      return cachedPythonExec;
    }
    ...
    if (await pathExists(systemPython)) {
      ...
      writeMCPLog(
        `[resolvePythonExec] Using system Python: ${systemPython}`,
        "Python Resolve",
      );
      return cachedPythonExec;
    }
  ...
  try {
    ...
    if (p) {
      ...
      writeMCPLog(
        `[resolvePythonExec] Using PATH Python: ${p}`,
        "Python Resolve",
      );
      return cachedPythonExec;
    }
  ...
  writeMCPLog(
    "[resolvePythonExec] No Python executable found!",
...
async function executePython(
  ...
  writeMCPLog(
    `[executePython] Python root: ${execInfo.pythonRoot}`,
    "Python Execution",
  );
  writeMCPLog(
    `[executePython] PYTHONHOME: ${env.PYTHONHOME || "not set"}`,
    "Python Execution",
  );
  writeMCPLog(
    `[executePython] PYTHONPATH: ${env.PYTHONPATH || "not set"}`,
    "Python Execution",
  );
  writeMCPLog(
    `[executePython] CONDA_PREFIX: ${env.CONDA_PREFIX || "not set"}`,
    "Python Execution",
  );
  writeMCPLog(
    `[executePython] Code length: ${code.length} chars`,
    "Python Execution",
  );
  ...
  return await new Promise<{ stdout: string; stderr: string }>(
    (resolve, reject) => {
      ...
      child.on("error", (err) => {
        clearTimeout(timer);
        writeMCPLog(
          `[executePython] Spawn failed: ${err.message}`,
          "Python Execution Error",
        );
      ...
      child.on("close", (code) => {
        clearTimeout(timer);
        writeMCPLog(
          `[executePython] Process closed with code: ${code}`,
          "Python Execution",
        );
        if (code === 0) {
          writeMCPLog(
...
async function performMacMouseMoveViaQuartz(
  ...
  const { cocoaX, cocoaY } = await convertCliclickToCocoaCoordinates(
    globalX,
    globalY,
  );
...
async function macReadClipboardBytes(
...
async function macWriteClipboardBytes(
  bytes: Buffer,
...
async function tryLocateElementInDockByAccessibility(
  ...
  for (const item of dockItems) {
    const score = scoreDockItemAgainstDescription(
      item.name,
      elementDescription,
    );
    if (score > bestScore) {
 ...
 */
async function executeCliclick(
  command: string,
): Promise<{ stdout: string; stderr: string }> {
  if (PLATFORM !== "darwin") {
    throw new Error(
      "cliclick is only available on macOS. Use Windows-specific functions instead.",
    );
  }
 ...
 */
async function windowsGetMousePosition(): Promise<{
  globalX: number;
  globalY: number;
}> {
  const script = `
 ...
 */
async function windowsMoveMouse(
  globalX: number,
  globalY: number,
): Promise<void> {
  const script = `
...
async function getDisplayConfiguration(): Promise<DisplayConfiguration> {
  ...
  if (
    displayConfigCache &&
    now - displayConfigCacheTime < DISPLAY_CONFIG_CACHE_TTL
  ) {
  ...
  if (PLATFORM !== "darwin") {
    throw new Error(
      `Display detection is not supported on platform: ${PLATFORM}`,
    );
  }
  ...
  try {
    ...
    try {
      const result = await executeCommandSafe("system_profiler", [
        "SPDisplaysDataType",
        "-json",
      ]);
      ...
      try {
        ...
        writeMCPLog(
          "[GUI] Failed to parse system_profiler JSON output",
          "Display Detection Error",
        );
      ...
      const config: DisplayConfiguration = {
        displays,
        totalWidth: displays.reduce(
          (max, d) => Math.max(max, d.originX + d.width),
          0,
        ),
        totalHeight: displays.reduce(
          (max, d) => Math.max(max, Math.abs(d.originY) + d.height),
          0,
        ),
...
async function resolveClickCoordinates(
  ...
  if (coordinateType === "auto") {
    const isOutOfBounds =
      x < 0 || y < 0 || x >= display.width || y >= display.height;
    const looksNormalized =
      xInput >= 0 && xInput <= 1000 && yInput >= 0 && yInput <= 1000;

    if (isOutOfBounds && looksNormalized) {
      const converted = await convertNormalizedToDisplayCoordinates(
        xInput,
        yInput,
        displayIndex,
      );
  ...
  const clampedX =
    display.width > 0 ? Math.max(0, Math.min(display.width - 1, x)) : x;
  const clampedY =
    display.height > 0 ? Math.max(0, Math.min(display.height - 1, y)) : y;
...
async function performClick(
  ...
  if (PLATFORM === "darwin") {
    ...
    const nearBottomDockZone = Boolean(
      targetDisplay &&
      localY >= Math.max(0, targetDisplay.height - dockZoneHeight),
    );
...
async function performType(
  ...
  const usePaste =
    inputMethod === "paste" || (inputMethod === "auto" && hasNonAscii);
    ...
    if (key.length === 1) {
      ...
      if (cliclickModifiers.length > 0) {
        ...
        if (keyCode !== undefined) {
          ...
          const modifierFlags: string[] = [];
          if (cliclickModifiers.includes("cmd"))
            modifierFlags.push("command down");
          if (cliclickModifiers.includes("ctrl"))
            modifierFlags.push("control down");
          if (cliclickModifiers.includes("shift"))
            modifierFlags.push("shift down");
          if (cliclickModifiers.includes("alt"))
            modifierFlags.push("option down");

          const usingClause =
            modifierFlags.length > 0
              ? ` using {${modifierFlags.join(", ")}}`
              : "";
...
async function performDrag(
  ...
  const fromCoords = await convertToGlobalCoordinates(
    fromX,
    fromY,
    displayIndex,
  );
  ...
  // Use formatCliclickCoords to handle negative coordinates (displays to left/above main)
  const fromCoordsStr = formatCliclickCoords(
    fromCoords.globalX,
    fromCoords.globalY,
  );
...
async function takeScreenshot(
  ...
  if (region) {
    ...
    screencaptureArgs.push(
      "-R",
      `${globalX},${globalY},${region.width},${region.height}`,
    );
  }
    ...
    for (const file of fsSync.readdirSync(SCREENSHOTS_DIR)) {
      ...
      try {
        ...
        if (now - stat.mtimeMs > maxAge) {
          ...
          writeMCPLog(
            `[Screenshot Cleanup] Deleted old screenshot: ${file}`,
            "Screenshot Cleanup",
          );
        }
      } catch {
...
async function takeScreenshotForDisplay(
  ...
  const reusable = forceRefresh
    ? null
    : getReusableScreenshot(normalizedDisplayIndex, regionKey);
  if (reusable) {
  ...
  const tempPath = path.join(
    SCREENSHOTS_DIR,
    `screenshot_display_${timestamp}.png`,
  );
  ...
  const config = await getDisplayConfiguration();
  const display =
    config.displays.find((d) => d.index === normalizedDisplayIndex) ||
    config.displays[0];
  ...
  const disableImageOutput =
    process.env.OPEN_COWORK_DISABLE_IMAGE_TOOL_OUTPUT === "1";
 ...
 */
async function getMousePosition(): Promise<{
  x: number;
  y: number;
  displayIndex: number;
}> {
...
async function callVisionAPI(
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    try {
      ...
      writeMCPLog(
        `${logPrefix} Attempt ${attempt}/${MAX_RETRIES} - Success`,
        "API Request",
      );
      return result;
    } catch (error: unknown) {
      ...
      if (isVisionRequestShapeError(errorMessage)) {
        if (!compatibilityFallbackUsed) {
          ...
          try {
            ...
            writeMCPLog(
              `${logPrefix} Compatibility fallback succeeded`,
              "API Request",
            );
            return compatResult;
      ...
      if (isLastAttempt) {
        writeMCPLog(
          `${logPrefix} All ${MAX_RETRIES} attempts failed`,
          "API Request Failed",
        );
        throw new Error(
          `Vision API failed after ${MAX_RETRIES} attempts: ${errorMessage}`,
        );
      }
      ...
      writeMCPLog(
        `${logPrefix} Waiting ${waitTime}ms before retry...`,
        "API Request Retry",
      );
...
async function callVisionAPIWithTimeout(
  ...
  if (!selectedApiKey) {
    ...
    throw new Error(
      "OpenAI API key not configured for vision route. Please set OPENAI_API_KEY.",
    );
  }
      ...
      const req = httpModule.request(options, (res: any) => {
        ...
        res.on("end", () => {
          ...
          if (res.statusCode && res.statusCode >= 200 && res.statusCode < 300) {
            ...
            reject(
              new Error(
                `API request failed: ${res.statusCode} ${res.statusMessage} - ${data}`,
              ),
            );
          }
        });
...
async function annotateScreenshotWithClickHistory(
  ...
  const remainingClicks = clickHistoryForDisplay.filter(
    (click) => click !== mostRecentClick,
  );
...
async function analyzeScreenshotWithVision(
  ...
  try {
    ...
    const { annotatedPath, clickHistoryInfo } =
      await annotateScreenshotWithClickHistory(
        screenshotPath,
        targetDisplay.index,
      );
...
{"box_2d": [ymin, xmin, ymax, xmax], "confidence": <0-100>}`;
    ...
    if (!jsonMatch) {
      ...
      throw new Error(
        "Failed to parse vision model response: No JSON found in response",
      );
    }
    ...
    try {
      ...
      writeMCPLog(
        `[analyzeScreenshotWithVision] JSON parsed successfully`,
        "JSON Parse Success",
      );
...
async function planGUIActions(
  ...
  if (PLATFORM !== "darwin" && PLATFORM !== "win32") {
    throw new Error(
      `GUI action planning is not supported on platform: ${PLATFORM}`,
    );
  }
...
- Instead of "menu", use "the File menu in the menu bar"`;
  ...
  writeMCPLog(
    `[planGUIActions] Raw Response Length: ${responseText.length}`,
    "Response",
  );
  writeMCPLog(
  ...
  if (!jsonMatch) {
    ...
    throw new Error(
      "Failed to parse action plan response: No JSON found in response",
    );
  }
...
async function locateGUIElement(
  ...
  if (PLATFORM !== "darwin" && PLATFORM !== "win32") {
    throw new Error(
      `Element location is not supported on platform: ${PLATFORM}`,
    );
  }
  ...
  try {
    ...
    if (targetDisplay) {
      ...
      writeMCPLog(
        `[locateGUIElement] Marked screenshot saved to: ${markedPath}`,
        "Image Marking",
      );
    }
...
async function executeActionStep(
  ...
  try {
    ...
    // Locate the element
    const coords = await locateGUIElement(
...
async function performVisionBasedInteraction(
  ...
  if (PLATFORM !== "darwin" && PLATFORM !== "win32") {
    throw new Error(
      `Vision-based GUI interaction is not supported on platform: ${PLATFORM}`,
    );
  }

  writeMCPLog(
    `[performVisionBasedInteraction] Starting task: "${taskDescription}"`,
    "Task Start",
  );
  writeMCPLog(
  ...
  writeMCPLog(
    `[performVisionBasedInteraction] Step 1: Planning actions...`,
    "Task Planning",
  );
  let plan;
  try {
  ...
  return JSON.stringify({
    ...
    results,
    failed_at_step: allSuccessful
      ? undefined
      : results.findIndex((r) => !r.success) + 1,
  });
}
...
async function verifyGUIState(
  ...
  if (PLATFORM !== "darwin" && PLATFORM !== "win32") {
    throw new Error(
      `GUI verification is not supported on platform: ${PLATFORM}`,
    );
  }
...
- Reason: The button was clicked correctly in the expected dialog window.`;
  ...
  let answer = await callVisionAPI(
    base64Image,
    prompt,
    20000,
    "verifyGUIState",
  ...
  if (PLATFORM === "darwin") {
    ...
    if (expectedAliases.length > 0 && requiresForegroundMatch) {
      ...
      if (frontmostApp) {
        const frontmostMatched = appNameMatchesAliases(
          frontmostApp,
          expectedAliases,
        );
        writeMCPLog(
...
Provide the extracted information in a clear, structured format. If extracting multiple items, organize them logically.`;
  ...
  const extractedInfo = await callVisionAPI(
    base64Image,
    prompt,
    30000,
    "extractGUIInfo",
  ...
  writeMCPLog(
    `[extractGUIInfo] Response Length: ${extractedInfo.length}`,
    "Response",
  );
  writeMCPLog(
  ...
  return {
    tools: [
        ...
        inputSchema: {
          ...
          properties: {
            ...
            x: {
              type: "number",
              description:
                "X coordinate (interpretation depends on coordinate_type)",
            },
            y: {
              type: "number",
              description:
                "Y coordinate (interpretation depends on coordinate_type)",
            },
            display_index: {
              type: "number",
              description:
        ...
        inputSchema: {
          ...
          properties: {
            ...
            from_x: {
              type: "number",
              description:
                "Starting X coordinate (normalized 0-1000 by default)",
            },
            from_y: {
              type: "number",
              description:
                "Starting Y coordinate (normalized 0-1000 by default)",
            },
            to_x: {
              type: "number",
      ...
      },
      {
        name: "screenshot",
        description:
          "Take a screenshot of the screen, a specific display, or a region.",
        inputSchema: {
          type: "object",
          properties: {
            output_path: {
            ...
            display_index: {
              type: "number",
              description:
                "Display index to capture. If not provided, captures all displays.",
            },
            region: {
              type: "object",
      ...
      },
      {
        ...
        description:
          "Get the current mouse cursor position, including which display it is on.",
        inputSchema: {
          type: "object",
          properties: {},
          required: [],
        ...
        inputSchema: {
          ...
          properties: {
            ...
            display_index: {
              type: "number",
              description:
                "Display index to search on. If not provided, uses main display.",
            },
          },
        ...
        inputSchema: {
          ...
          properties: {
            ...
            display_index: {
              type: "number",
              description:
                "Display index to verify. If not provided, uses main display.",
            },
          },
          required: ["question"],
        },
        ...
        inputSchema: {
          ...
          properties: {
            ...
            display_index: {
              type: "number",
              description:
                "Display index to capture. If not provided, uses main display.",
            },
          },
  ...
  try {
    ...
    switch (name) {
      ...
      case "click": {
        ...
        const resolved = await resolveClickCoordinates(
          x,
          y,
          display_index,
          coordinate_type,
        );
        result = await performClick(
          resolved.x,
          resolved.y,
          display_index,
          click_type,
      ...
      case "gui_locate_element": {
        ...
        const location = await locateGUIElement(
          element_description,
          display_index,
        );
      ...
      case "gui_interact_vision": {
        ...
        result = await performVisionBasedInteraction(
          task_description,
          display_index,
        );
        break;
