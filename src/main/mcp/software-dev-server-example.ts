...
async function buildDockerGUITestImage(
  config: DockerGUITestConfig,
): Promise<string> {
  // Validate config values to prevent injection in Dockerfile template
  if (
    !Number.isInteger(config.vncPort) ||
    config.vncPort < 1024 ||
    config.vncPort > 65535
  ) {
    throw new Error(
...
async function startGUIApplicationInDocker(
  ...
  writeMCPLog(
    "[Docker] Starting GUI application in isolated Docker environment...",
  );
  ...
  try {
    ...
    if (!/^[a-f0-9]{12,64}$/.test(containerId)) {
      throw new Error(
        `Invalid container ID returned from docker run: ${containerId}`,
      );
    }
    ...
    if (enableVnc) {
      ...
      if (!vncRunning) {
        writeMCPLog(
          "[Docker] Warning: VNC server may not be running. Check container logs.",
        );
      }
...
async function stopGUIApplication(
  ...
  if (instance.isDocker && instance.containerId) {
    writeMCPLog(
      `[Docker] Stopping container: ${instance.containerId.substring(0, 12)}`,
    );
...
async function getDockerContainerLogs(
  containerId: string,
  tail: number = 0,
): Promise<string> {
  try {
...
async function saveDockerDiagnostics(
  ...
  const logFile = path.join(
    logDir,
    `container-${containerId.substring(0, 12)}-${timestamp}.log`,
  );
...
async function takeScreenshot(outputPath: string): Promise<string> {
  ...
  if (currentGUIApp && currentGUIApp.isDocker && currentGUIApp.containerId) {
    writeMCPLog(
      "[Screenshot] Taking screenshot from Docker container Xvfb display...",
    );
    ...
    try {
      ...
      writeMCPLog(
        `[Screenshot] Screenshot copied from container to ${outputPath}`,
      );
  ...
  try {
    ...
    if (platform === "darwin") {
      ...
      try {
        ...
        writeMCPLog(
          "[Vision] Failed to get macOS screen resolution, using default",
        );
      }
      ...
      try {
        ...
        try {
          ...
          writeMCPLog(
            "[Vision] Failed to get Linux screen resolution, using default",
          );
        }
      }
    }
  ...
  try {
    ...
    if (
      ...
      const timeSinceUpdate =
        Date.now() - currentScreenContext.lastUpdated.getTime();
...
Be PRECISE with coordinates. Measure carefully from the top-left corner.`;
    ...
    writeMCPLog(
      `[Vision] Screen context built: ${context.elements.length} elements identified`,
    );
  ...
  try {
    if (platform === "darwin") {
      ...
      if (appName) {
        ...
        writeMCPLog(
          `[GUI] AppleScript result - stdout: ${stdout}, stderr: ${stderr}`,
        );
      } else {
        ...
        try {
          ...
          writeMCPLog(
            `[GUI] AppleScript (Python) result - stdout: ${stdout}, stderr: ${stderr}`,
          );
        } catch (err1: unknown) {
          writeMCPLog(
          ...
          try {
            ...
            writeMCPLog(
              "[GUI] Successfully focused python process (lowercase)",
            );
          } catch (err2: unknown) {
            writeMCPLog(
            ...
            if (currentGUIApp && currentGUIApp.pid) {
              try {
                ...
                writeMCPLog(
                  `[GUI] Successfully focused process by PID: ${currentGUIApp.pid}`,
                );
              } catch (err3: unknown) {
                writeMCPLog(
      ...
      writeMCPLog(
        `[GUI] PowerShell result - stdout: ${stdout}, stderr: ${stderr}`,
      );
    } else {
      ...
      try {
        if (appName) {
          ...
          writeMCPLog(
            `[GUI] xdotool result - stdout: ${stdout}, stderr: ${stderr}`,
          );
        } else {
          const { stdout, stderr } = await execFileAsync(
            "xdotool",
        ...
        writeMCPLog(
          "[GUI] Please install xdotool: sudo apt-get install xdotool",
        );
      }
    }
  ...
  try {
    ...
    if (coords.confidence < 50) {
      return {
        ...
        suggestion:
          "Try a more specific description or check if the element is visible",
      };
    }
    ...
    if (currentGUIApp.isDocker && currentGUIApp.containerId) {
      ...
      switch (action) {
        case "click":
          await executeCommand(
            `xdotool mousemove ${coords.x} ${coords.y} click 1`,
          );
          await new Promise((resolve) => setTimeout(resolve, 500));
        ...
        case "double_click":
          await executeCommand(
            `xdotool mousemove ${coords.x} ${coords.y} click --repeat 2 1`,
          );
          await new Promise((resolve) => setTimeout(resolve, 500));
        ...
        case "right_click":
          await executeCommand(
            `xdotool mousemove ${coords.x} ${coords.y} click 3`,
          );
          await new Promise((resolve) => setTimeout(resolve, 500));
      ...
      return {
        ...
        suggestion:
          "Use macOS (cliclick) or Linux (xdotool) for vision-based GUI automation",
      };
    }
  } catch (error: unknown) {
    return {
      ...
      switch (action) {
        ...
        case "move":
          if (x !== undefined && y !== undefined) {
            ...
            return {
              success: false,
              message: "Coordinates required for move action",
            };
          }
        ...
        case "type": {
          if (!value) {
            return {
              success: false,
              message: "Value required for type action",
            };
          }
        ...
        case "drag": {
          if (!value) {
            return {
              success: false,
              message:
                'Coordinates required for drag action (format: "x1,y1,x2,y2")',
            };
          }
        ...
        default:
          return {
            success: false,
            message: `Action '${action}' is not supported in Docker mode`,
          };
      }
    }
        ...
        inputSchema: {
          ...
          properties: {
            app_file_path: {
              type: "string",
              description:
                "Path to the application file to run (e.g., app.py, index.html)",
            },
            app_type: {
              type: "string",
            ...
            vnc_port: {
              type: "number",
              description:
                "VNC port to expose (default: 5901, only for Docker mode)",
            },
          },
        ...
        inputSchema: {
          ...
          properties: {
            action: {
              ...
              description:
                "Action to perform. Use coordinates (x, y) for click/move actions.",
            },
            x: {
              type: "number",
        ...
        inputSchema: {
          ...
          properties: {
            ...
            expected_answer: {
              type: "string",
              description:
                'Expected answer (e.g., "yes", "true", "Hello World")',
            },
          },
          required: ["question"],
        },
        ...
        inputSchema: {
          ...
          properties: {
            save_to_file: {
              type: "boolean",
              description:
                "Save logs to file in .docker-logs directory (default: true)",
            },
          },
        },
      },
  ...
  try {
    switch (name) {
      case "create_requirement": {
        const { description, files } = args as {
          description: string;
          files?: string[];
        };
      ...
      case "update_requirement": {
        const { requirement_id, updated_description, reason, status } =
          args as {
      ...
      case "gui_interact": {
        ...
        if (!currentGUIApp) {
          throw new Error(
            "No GUI application is running. Use start_gui_application first.",
          );
        }
        ...
        try {
          const result = await executeGUIInteraction(
            action,
            x,
            y,
            value,
      ...
      case "get_docker_logs": {
        ...
        if (
          !currentGUIApp ||
          !currentGUIApp.isDocker ||
          !currentGUIApp.containerId
        ) {
          return {
            content: [
              {
        ...
        try {
          ...
          if (save_to_file !== false) {
            logFile = await saveDockerDiagnostics(
              currentGUIApp.containerId,
              WORKSPACE_DIR,
            );
          }
      ...
      case "gui_interact_vision": {
        ...
        writeMCPLog(
          `[Vision] Performing ${action} on "${element_description}"`,
        );
      ...
      case "gui_verify_vision": {
        ...
        try {
          ...
          if (!currentGUIApp.isDocker) {
            writeMCPLog(
              "[Vision] Bringing window to front for verification...",
            );
            await focusApplicationWindow();
            writeMCPLog(
              "[Vision] Waiting 1 second for window to come to front...",
            );
            await new Promise((resolve) => setTimeout(resolve, 1000));
          }
...
async function main() {
  ...
  writeMCPLog(
    "    - gui_interact: Direct coordinate-based interaction (cliclick/xdotool)",
  );
