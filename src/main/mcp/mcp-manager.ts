...
export class MCPManager {
  ...
  private getBundledNodePath(): { node: string; npx: string } | null {
    ...
    if (!app.isPackaged) {
      ...
      log(
        "[MCPManager] Development mode, using downloaded node in resources/node",
      );
  ...
  private async resolvePreferredNpxPath(
    pathEnv: string | undefined,
  ): Promise<string> {
    ...
    if (process.platform === "win32") {
      ...
      if (!preferredNpxPath) {
        throw new Error(
          "npx is not available. Install Node.js so OMAGT can use your system npx.cmd, or reinstall the app to restore the bundled runtime.",
        );
      }
  ...
  private async getEnhancedEnv(
    ...
    return { ...this.cachedBaseEnv, ...legacyBridgeEnv, ...configEnv };
  }
    ...
    if (platform === "darwin" || platform === "linux") {
      try {
        ...
        if (shellEnv.PATH && process.env.PATH) {
          ...
          const processPaths = process.env.PATH.split(pathDelimiter).filter(
            (p) => p.trim(),
          );
      ...
      await Promise.allSettled(
        enabledConfigs.map(async (config) => {
          try {
            ...
            logError(
              `[MCPManager] Failed to connect to server ${config.name}:`,
              error,
            );
          }
        }),
  ...
  async updateServer(config: MCPServerConfig): Promise<void> {
    ...
    log(
      `[MCPManager] Updating server: ${config.name} (enabled: ${config.enabled})`,
    );
    ...
    if (config.enabled && !isConnected) {
      ...
      try {
        ...
        logError(
          `[MCPManager] Failed to reconnect server ${config.name}:`,
          error,
        );
        throw error;
      }
  ...
  private getMcpServerPath(filename: string): string {
    ...
    try {
      if (fs.existsSync(devBundledPath)) {
        log(
          `[MCPManager] Found bundled MCP server (dev) at: ${devBundledPath}`,
        );
        return devBundledPath;
      }
    } catch (error) {
      logWarn(
        `[MCPManager] Error checking dev bundled MCP server path: ${error}`,
      );
    }
  ...
  private async connectServer(config: MCPServerConfig): Promise<void> {
    log(
      `[MCPManager] Connecting to MCP server: ${config.name} (${config.type})`,
    );
  ...
  private async connectServerInternal(config: MCPServerConfig): Promise<void> {
    ...
    if (config.type === "stdio") {
      ...
      if (isBuiltinServer && isOldConfig && app.isPackaged) {
        log(
          `[MCPManager] Auto-migrating ${config.name} from npx/tsx to node (production mode)`,
        );
      ...
      if (!app.isPackaged && isBuiltinServer) {
        ...
        const tsScript = args.find(
          (a) => typeof a === "string" && a.endsWith(".ts"),
        );
      ...
      if (process.platform === "win32") {
        ...
        if (winSuffixMap[cmdBase] && command === cmdBase) {
          ...
          log(
            `[MCPManager] Windows: resolved bare command '${cmdBase}' to '${command}'`,
          );
        }
      }
      ...
      log("[MCPManager] Server auth env summary", {
        ...
        ANTHROPIC_AUTH_TOKEN: env.ANTHROPIC_AUTH_TOKEN?.trim()
          ? "set"
          : "unset",
      });
      ...
      log(
        `[MCPManager] Creating STDIO transport: ${command} ${args.join(" ")}`,
      );
      log(
        `[MCPManager] Environment variables: ${Object.keys(env).length} vars`,
      );
      ...
      try {
        ...
        throw new Error(
          `SSE server ${config.name} has a malformed URL: "${config.url}"`,
        );
      }
    ...
    try {
      ...
      const connectTimeoutPromise = new Promise<never>((_, reject) => {
        connectTimeoutId = setTimeout(
          () =>
            reject(
              new Error(
                `MCP server connection timed out after ${connectTimeoutMs / 1000}s`,
              ),
            ),
          connectTimeoutMs,
        );
      ...
      if (config.type === "stdio") {
        try {
          ...
          if (transportAny._process) {
            ...
            log(
              `[MCPManager] MCP server process spawned with PID: ${childProcess.pid}`,
            );
            ...
            if (childProcess.stderr) {
              childProcess.stderr.on("data", (data: Buffer) => {
                try {
                  ...
                  logError(
                    "[MCPManager] Error processing MCP server stderr:",
                    error,
                  );
                }
              });
            ...
            childProcess.on("exit", (code: number, signal: string) => {
              if (code !== null && code !== 0) {
                logError(
                  `[MCPManager] MCP server process exited with code ${code}`,
                );
              } else if (signal) {
                logError(
                  `[MCPManager] MCP server process killed with signal ${signal}`,
                );
              } else {
            ...
            childProcess.on("error", (error: Error) => {
              logError(
                `[MCPManager] MCP server process error: ${error.message}`,
              );
      ...
      if (config.type === "stdio" && commandForLogging) {
        logError(
          `[MCPManager] STDIO transport may have failed to spawn process or communicate`,
        );
        logError(
          `[MCPManager] Command was: ${commandForLogging} ${argsForLogging.join(" ")}`,
        );
      }
  ...
  private async isChromeDebugPortReady(): Promise<boolean> {
    try {
      log(
        `[MCPManager] Checking Chrome debug port: http://localhost:9222/json/version`,
      );
      ...
      if (response.ok) {
        ...
        log(
          `[MCPManager] Chrome debug port returned status: ${response.status}`,
        );
        return false;
      }
    } catch (error: unknown) {
  ...
  private async waitForChromeDebugPort(
    ...
    log(
      `[MCPManager] Waiting for Chrome debug port (max ${maxRetries} retries)...`,
    );
    ...
    for (let i = 0; i < maxRetries; i++) {
      ...
      if (i < maxRetries - 1) {
        log(
          `[MCPManager] Port not ready, retrying in ${delayMs}ms... (${i + 1}/${maxRetries})`,
        );
    ...
    logWarn(
      `[MCPManager] Chrome debug port not ready after ${maxRetries} attempts`,
    );
    return false;
  }
  ...
  private async ensureChromeReady(
    ...
    log(
      `[MCPManager] Step 1: Checking if Chrome debug port 9222 is accessible...`,
    );
    ...
    if (portReady) {
      ...
      try {
        ...
        log(
          `[MCPManager] ✓ Chrome connected successfully, using existing instance`,
        );
        ...
        logWarn(
          `[MCPManager] Error code: ${chromeErr.code}, message: ${chromeErr.message}`,
        );
      ...
      log(
        `[MCPManager] Will start new Chrome instance with debugging enabled...`,
      );
    }
    ...
    try {
      ...
      log(
        `[MCPManager] Step 3: Waiting for Chrome debug port to become ready...`,
      );
      const portBecameReady = await this.waitForChromeDebugPort(15, 1000);

      if (!portBecameReady) {
        logError(
          `[MCPManager] ❌ Chrome debug port did not become ready after 15 seconds`,
        );
      ...
      for (let i = 0; i < 5; i++) {
        try {
          ...
          if (i < 4) {
            log(
              `[MCPManager] Connection verification attempt ${i + 1}/5 failed, retrying...`,
            );
            ...
            logError(
              `[MCPManager] Last error code: ${ve.code}, message: ${ve.message}`,
            );
            logError(
  ...
  async disconnectServer(serverId: string): Promise<void> {
    ...
    if (transport) {
      try {
        ...
        logError(
          `[MCPManager] Error closing transport for ${serverId}:`,
          error,
        );
      }
      ...
      try {
        ...
        const timeoutPromise = new Promise<never>((_, reject) => {
          timeoutId = setTimeout(
            () => reject(new Error("listTools timeout after 10s")),
            timeoutMs,
          );
        });

        log(
          `[MCPManager] Fetching tools from ${config.name} (timeout: ${timeoutMs}ms)...`,
        );

        let listToolsResult;
        try {
          listToolsResult = await Promise.race([
            listToolsPromise,
            timeoutPromise,
          ]);
        ...
        log(
          `[MCPManager] ✓ Loaded ${listToolsResult.tools.length} tools from ${config.name}`,
        );
      } catch (error: unknown) {
        const errMsg = error instanceof Error ? error.message : String(error);
        logError(
          `[MCPManager] ❌ Error listing tools from ${serverId}:`,
          errMsg,
        );
        ...
        if (config && config.name.toLowerCase().includes("chrome")) {
          log(
            `[MCPManager] Chrome server may need reconnection. Trying to refresh...`,
          );
        }
      }
    }
  ...
  async callTool(
    ...
    logCtx(
      `[MCPManager] Calling tool ${actualToolName} on server ${tool.serverName}`,
    );
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
      ...
      try {
        ...
        if (
          ...
          shouldHotReloadGuiVisionServer(
        ...
        if (errorMsg.includes("timeout")) {
          log(
            `[MCPManager] Tool call timeout detected, retrying after backoff...`,
          );
          const delay = Math.min(2000 * Math.pow(1.5, attempt), 10000);
          await new Promise((resolve) => setTimeout(resolve, delay));
          continue;
  ...
  private async reconnectServer(serverId: string): Promise<boolean> {
    ...
    if (!config || !config.enabled) {
      logWarn(
        `[MCPManager] Cannot reconnect server ${serverId}: config missing or disabled`,
      );
      return false;
    }
