  ...
  try {
    ...
    if (files.length > MAX_LOG_FILES) {
      ...
      const filesToDelete = files
        .slice(MAX_LOG_FILES)
        .filter(
          (file) => !activeLogFilePath || file.path !== activeLogFilePath,
        );
...
function rotateLogIfNeeded(): void {
  ...
  try {
    ...
    if (stats.size > MAX_LOG_SIZE) {
      safeConsoleLog(
        `[Logger] Log file size (${stats.size}) exceeds limit, rotating...`,
      );
...
function normalizeLogValue(
  ...
  if (value instanceof Error) {
    ...
    return {
      ...
      stack: err.stack
        ? truncateLogText(err.stack, MAX_LOG_STRING_LENGTH * 2)
        : undefined,
      cause:
        err.cause !== undefined
          ? normalizeLogValue(err.cause, seen, depth + 1)
          : undefined,
      meta:
...
export function shouldPersistLogLevel(
  level: string,
 ...
 * Get all log files
 */
export function getAllLogFiles(): Array<{
  name: string;
  path: string;
  size: number;
  mtime: Date;
    ...
    return fs
      ...
      .flatMap((f) => {
        ...
        try {
          ...
          return [
            { name: f, path: filePath, size: stats.size, mtime: stats.mtime },
          ];
        } catch {
