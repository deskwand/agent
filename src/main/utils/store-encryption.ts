...
function buildLegacyDirCandidates(moduleDirname: string): string[] {
  ...
  if (process.resourcesPath) {
    candidates.push(
      path.join(process.resourcesPath, "app.asar", "dist-electron", "main"),
    );
  }
...
export const SECURE_SCRYPT_OPTIONS: crypto.ScryptOptions = createScryptOptions(
  65536,
  8,
  1,
);
...
export const LEGACY_SCRYPT_OPTIONS: crypto.ScryptOptions = createScryptOptions(
  16384,
  8,
  1,
);
...
function isLikelyKeyMismatch(error: unknown): boolean {
  ...
  return /\bUnexpected token\b|\bvalid JSON\b|\bbad decrypt\b|\bdecrypt\b|\bJSON\b/i.test(
    message,
  );
}
...
function buildBackupPath(
  storePath: string,
  reason: string = "pre-key-rotation",
): string {
...
export function getLegacyDerivedKeyHexes(
...
export function getLegacyDerivedKeyBuffers(
  options: KeyMaterialOptions,
): Buffer[] {
  return buildLegacyDirCandidates(options.moduleDirname).map((dir) =>
    deriveKeyBuffer(
...
export function createEncryptedStoreWithKeyRotation<
  T extends Record<string, unknown>,
>(options: EncryptedStoreRotationOptions<T>): Store<T> {
