...
export interface DiagnosticsSummary {
  ...
  config: {
    ...
    omagtCodePathConfigured: boolean;
...
export function redactFileSystemPath(value?: string | null): string | null {
  ...
  const normalizedForTmpCheck = normalizePathSeparators(
  ...
  if (/AppData[/\\]Local[/\\]Temp/i.test(trimmed)) {
    const appDataTempIdx =
      normalizedForTmpCheck.search(/appdata\/local\/temp/i);
    if (appDataTempIdx >= 0) {
...
function summarizeMessageMeta(message?: Message): MessageMetaSummary | null {
  ...
  return {
    ...
    textBlockCount: message.content.filter((block) => block.type === "text")
      .length,
    imageBlockCount: message.content.filter((block) => block.type === "image")
      .length,
    fileAttachmentCount: message.content.filter(
      (block) => block.type === "file_attachment",
    ).length,
    toolUseCount: message.content.filter((block) => block.type === "tool_use")
      .length,
    toolResultCount: message.content.filter(
...
export function summarizeTraceStepMeta(step: TraceStep): TraceStepMetaSummary {
  return {
    ...
    toolInputKeys: step.toolInput
      ? Object.keys(step.toolInput).slice(0, 12)
      : [],
...
function buildSessionDiagnosticSummary(
  ...
  const recentUserMessage = recentMessages.find(
    (message) => message.role === "user",
  );
  const recentAssistantMessage = recentMessages.find(
    (message) => message.role === "assistant",
  );
  const errorSteps = traceSteps.filter(
    (step) => step.isError || step.status === "error",
  );
  ...
  return {
    ...
    latestErrorStep: latestErrorStep
      ? summarizeTraceStepMeta(latestErrorStep)
      : null,
  };
}

export function buildDiagnosticsSummary(
  input: BuildDiagnosticsSummaryInput,
): DiagnosticsSummary {
  ...
  return {
    ...
    runtime: {
      ...
      logsDirectory:
        redactFileSystemPath(input.runtime.logsDirectory) ||
        input.runtime.logsDirectory,
    },
    config: {
      ...input.config,
