...
async function requestBody(
  ...
  throw new Error("Unsupported request body for secure remote fetch");
}
    ...
    const request = (url.protocol === "https:" ? httpsRequest : httpRequest)(
      {
        ...
        agent: false,
        ...(url.protocol === "https:" && net.isIP(url.hostname) === 0
...
async function fetchResolvedUrl(
  ...
  for (const address of addresses) {
    try {
      return await requestAddress(url, init, address);
    } catch (error) {
  ...
  throw lastError instanceof Error
    ? lastError
    : new Error(`Could not connect to ${url.hostname}`);
}
...
export async function fetchRemoteUrl(
  ...
  let current = await resolveRemoteUrl(url, options);
  let requestInit = init;

  for (let redirects = 0; redirects <= maxRedirects; redirects++) {
    const finalInit = { ...requestInit, redirect: "manual" as const };
    const response = options.fetchResolved
      ? await options.fetchResolved(current.url, finalInit, current.addresses)
      : options.fetch
        ? await options.fetch(current.url, finalInit)
        : await fetchResolvedUrl(current.url, finalInit, current.addresses);
    ...
    await response.body?.cancel().catch(() => undefined);
    current = await resolveRemoteUrl(new URL(location, current.url), options);
    if (
      response.status === 303 ||
