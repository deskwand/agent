...
function redactLargeBinaryData(value: unknown): unknown {
  ...
  for (const [key, nestedValue] of Object.entries(value)) {
    if (typeof nestedValue === "string") {
      ...
      if (
        (key === "base64" || key === "inlineDataBase64") &&
        nestedValue.length > 128
      ) {
...
export function normalizeMcpToolResultForModel(
  ...
  if (resultObj?.content) {
    const { textParts, images } = extractTextAndImagesFromContent(
      resultObj.content,
    );
    return {
...
export function normalizeToolExecutionResultForUi(
  ...
  if (resultObj?.content) {
    const { textParts, images: inlineImages } = extractTextAndImagesFromContent(
      resultObj.content,
    );
