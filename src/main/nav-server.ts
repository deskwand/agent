 ...
 *   GET /status
 */
export function startNavServer(
    ...
    try {
      ...
      if (pathname === "/navigate") {
        ...
        if (page === "session" && !sessionId) {
          return json(res, 400, {
            ok: false,
            error: "session page requires id param",
          });
        }
        ...
        try {
          ...
          return json(res, 500, {
            ok: false,
            error: "Failed to execute navigation",
          });
        }

        return json(res, 200, {
          ok: true,
          navigated: { page, tab, sessionId },
        });
      }
      ...
      if (pathname === "/status") {
        ...
        try {
          ...
          return json(res, 500, {
            ok: false,
            error: "Failed to read renderer state",
          });
        }
      }
      ...
      json(res, 404, {
        ok: false,
        error: "Not found. Use /navigate or /status",
      });
    } catch (err) {
