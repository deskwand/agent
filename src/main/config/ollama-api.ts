...
const modelIndexCache = new Map<
  string,
  { expiresAt: number; result: OllamaModelIndexResult }
>();
...
async function parseJsonResponse(
  ...
  try {
    ...
    throw new Error(
      `Failed to parse Ollama API response: ${text.substring(0, 200)}`,
    );
  }
}
...
export async function fetchOllamaModelIndex(input: {
  ...
  const request = (async (): Promise<OllamaModelIndexResult> => {
    ...
    const models = (Array.isArray(data?.data) ? data.data : [])
      ...
      .filter((item: ProviderModelInfo | null): item is ProviderModelInfo =>
        Boolean(item),
      );
  ...
  try {
    if (input.useLiveRequest) {
      ...
      const response = await fetch(
        `${buildBaseUrl(input.baseUrl)}/chat/completions`,
        {
          method: "POST",
          headers: buildHeaders(input.apiKey),
          body: JSON.stringify({
