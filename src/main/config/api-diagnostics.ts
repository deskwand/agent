...
function resolveEffectiveUrl(input: DiagnosticInput): URL {
  ...
  if (!raw && input.provider !== "custom") {
    raw = (PROVIDER_PRESETS as unknown as Record<string, { baseUrl?: string }>)[
      input.provider
    ]?.baseUrl;
  }
...
function getModelDiagnosticFix(
  errorType: Awaited<ReturnType<typeof probeWithAgentSdk>>["errorType"],
  model: string,
): string {
  switch (errorType) {
    case "unauthorized":
...
async function stepModel(
  ...
  const start = Date.now();
  try {
    const verificationLevel: DiagnosticVerificationLevel =
      input.verificationLevel ?? "deep";
    ...
    const result = await probeWithAgentSdk(
      {
        provider: input.provider,
        apiKey: input.apiKey,
        baseUrl: input.baseUrl,
...
async function runDiagnosticsImpl(
  input: DiagnosticInput,
): Promise<DiagnosticResult> {
  const verificationLevel: DiagnosticVerificationLevel =
    input.verificationLevel ?? "deep";
  ...
  const port = url.port
    ? Number(url.port)
    : defaultPort(url.protocol, input.provider, hostname);
