...
export class ToolExecutor {
  ...
  private resolveWorkspacePath(sessionId: string, inputPath: string): string {
    ...
    const candidate = path.normalize(
      path.join(normalizedPrimary, trimmed || "."),
    );
  ...
  async webSearch(query: string): Promise<string> {
    ...
    const collectTopics = (topic: unknown): void => {
      ...
      const firstUrl =
        typeof record.FirstURL === "string" ? record.FirstURL : "";
      if (text) {
   ...
   */
  private validateCommandSandbox(
    sessionId: string,
    command: string,
    cwd: string,
  ): void {
    ...
    for (const p of extractedPaths) {
      const isAbsolute =
        path.isAbsolute(p) || /^[A-Za-z]:/.test(p) || isUncPath(p);
      ...
      if (!allowed) {
        throw new Error(
          `Command blocked: path "${p}" is outside the mounted workspace`,
        );
      }
    }
  ...
  async searchFiles(
    ...
    try {
      const pathToSearch = this.resolveWorkspacePath(
        sessionId,
        searchPath || ".",
      );
      ...
      if (contentSearch) {
        ...
        if (pattern.startsWith("/") || pattern.startsWith("..")) {
          throw new Error(
            "Search pattern must be relative and cannot start with / or ..",
          );
        }
  ...
  async glob(
    ...
    try {
      ...
      if (pattern.startsWith("/") || pattern.startsWith("..")) {
        throw new Error(
          "Glob pattern must be relative and cannot start with / or ..",
        );
      }
      ...
      throw new Error(
        `Glob failed: ${error instanceof Error ? error.message : "Unknown error"}`,
      );
    }
  }
  ...
  private async searchDirectoryContents(
    ...
    for (const entry of entries) {
      ...
      if (entry.isDirectory()) {
        ...
        await this.searchDirectoryContents(
          fullPath,
          regex,
          results,
          maxResults,
        ...
        try {
          ...
          lines.forEach((line, index) => {
            if (results.length < maxResults && regex.test(line)) {
              results.push(
                `${fullPath}:${index + 1}: ${line.trim().slice(0, 100)}`,
              );
            }
            regex.lastIndex = 0;
          });
  ...
  private async read(
    ...
    try {
      const realPath = this.pathResolver.resolve(
        context.sessionId,
        virtualPath,
      );
      if (!realPath) {
