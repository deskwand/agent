...
describe("normalizeOpenAICompatibleBaseUrl", () => {
  ...
  it("preserves bare host URL without appending /v1", () => {
    expect(
      normalizeOpenAICompatibleBaseUrl("https://my-proxy.example.com"),
  ...
  it("strips /v1/completions suffix", () => {
    expect(
      normalizeOpenAICompatibleBaseUrl("https://api.openai.com/v1/completions"),
    ).toBe("https://api.openai.com/v1");
  });
  ...
  it("strips /v1/responses suffix", () => {
    expect(
      normalizeOpenAICompatibleBaseUrl("https://api.openai.com/v1/responses"),
    ).toBe("https://api.openai.com/v1");
  });
  ...
  it("preserves MSRA relay URL with /v1 suffix", () => {
    expect(
      normalizeOpenAICompatibleBaseUrl(
        "https://msra-im-relay.servicebus.windows.net/coproxy/v1",
      ),
