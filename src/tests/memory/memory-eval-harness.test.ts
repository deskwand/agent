...
class EvalMockLLM implements MemoryLLMClientLike {
  async complete(request: MemoryCompletionRequest): Promise<{ text: string }> {
    ...
    if (
      ...
      if (transcript.includes("gateway token rotation")) {
        return {
          text: JSON.stringify({
            session_summary:
              "记录 gateway token rotation 与 remote gateway 约束",
    ...
    if (request.systemPrompt.includes("strict memory retrieval evaluator")) {
      return {
        text: JSON.stringify({
          score: request.userPrompt.includes("gateway token rotation")
            ? 0.9
            : 0.8,
          reason: "good",
        }),
    ...
    if (
      request.systemPrompt.includes("optimizing prompts for a memory system")
    ) {
      return {
        text: JSON.stringify({
          candidates: [
...
describe("MemoryEvalHarness and MemoryPromptOptimizer", () => {
  ...
  beforeEach(() => {
    ...
    const runtimeConfig = mockConfigState.config
      .memoryRuntime as unknown as MemoryRuntimeConfig;
  ...
  it("uses the configured eval artifacts root when no artifactDir is passed", async () => {
    ...
    expect(report.artifactDir).toContain(
      path.join(tempRoot, "memory-root", "artifacts"),
    );
    expect(path.basename(report.artifactDir)).toMatch(/^memory-eval-/);
    expect(fs.existsSync(path.join(report.artifactDir, "report.json"))).toBe(
      true,
    );
  });
  ...
  it("iterates prompt candidates and keeps the best score", async () => {
    ...
    const result = await optimizer.optimize({
      ...
      evaluate: async (prompts) => ({
        ...baselineReport,
        averageScore:
          prompts.coreMemoryUpdateSystemPrompt === "candidate core prompt"
            ? 0.8
            : 0.4,
      }),
    });
    ...
    expect(result.prompts.coreMemoryUpdateSystemPrompt).toBe(
