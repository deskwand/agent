...
const itIfDirectorySymlinkSupported = supportsDirectorySymlinks()
  ? it
  : it.skip;
...
describe("dangling symlink cleanup in skill directories", () => {
  ...
  itIfDirectorySymlinkSupported(
    "fs.existsSync returns false for dangling symlinks",
    () => {
  ...
  itIfDirectorySymlinkSupported(
    "fs.mkdirSync fails on path with dangling symlink",
    () => {
      const linkPath = path.join(tmpDir, "docx");
      fs.symlinkSync(path.join(tmpDir, "missing-asar-path"), linkPath, "dir");
  ...
  itIfDirectorySymlinkSupported(
    "readdirSync lists dangling symlinks by name",
    () => {
      const linkPath = path.join(tmpDir, "docx");
      fs.symlinkSync(path.join(tmpDir, "missing-asar-path"), linkPath, "dir");
  ...
  itIfDirectorySymlinkSupported(
    "statSync throws ENOENT on dangling symlinks",
    () => {
      const linkPath = path.join(tmpDir, "docx");
      fs.symlinkSync(path.join(tmpDir, "missing-asar-path"), linkPath, "dir");
