...
export function useActiveSessionMessages(): Message[] {
  return useAppStore((s) =>
    s.activeSessionId
      ? (s.sessionStates[s.activeSessionId]?.messages ?? [])
      : [],
  );
}
 ...
 */
export function useActivePartialContent(): {
...
export function useActiveTurn(): TurnState | null {
  return useAppStore((s) =>
    s.activeSessionId
      ? (s.sessionStates[s.activeSessionId]?.activeTurn ?? null)
      : null,
  );
}

/** Returns the list of pending turn message IDs for the active session. */
export function usePendingTurns(): TurnState[] {
  return useAppStore((s) =>
    s.activeSessionId
      ? (s.sessionStates[s.activeSessionId]?.pendingTurns ?? [])
      : [],
  );
}
  ...
  return useAppStore(
    useShallow((s) => {
      ...
      const pendingCount = id
        ? (s.sessionStates[id]?.pendingTurns ?? []).length
        : 0;
      return {
        isRunning,
        hasActiveTurn,
...
export function useActiveExecutionClock(): SessionExecutionClock | undefined {
  return useAppStore((s) =>
    s.activeSessionId
      ? s.sessionStates[s.activeSessionId]?.executionClock
      : undefined,
  );
}
...
export function useActiveTraceSteps(): TraceStep[] {
  return useAppStore((s) =>
    s.activeSessionId
      ? (s.sessionStates[s.activeSessionId]?.traceSteps ?? [])
      : [],
  );
}
...
export function useActiveContextWindow(): number | undefined {
  return useAppStore((s) =>
    s.activeSessionId
      ? s.sessionStates[s.activeSessionId]?.contextWindow
      : undefined,
  );
}
...
/** Returns sidebar collapsed flag for layout components. */
export function useLayoutState(): { sidebarCollapsed: boolean } {
  return useAppStore(
    useShallow((s) => ({
