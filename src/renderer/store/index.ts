...
export interface SessionState {
  ...
  /** 当前生效的任务清单。null = 未知/没有；[] = 没有当前计划（模型显式清空，或新一轮开始）。 */
  currentTodos: CurrentTodos | null;
  /** 当前清单是否被声明「结束」（`done: true`）。清单为空 / 未知时恒为 false。 */
  ...
  /** 被记住那份清单**自己**声明的 done；后续清空调用不得覆盖它。 */
...
export const useAppStore = create<AppState>((set, get) => ({
  ...
  addMessage: (sessionId, message) =>
    set((state) => {
      ...
      // 被拒的清单会被 collectCurrentPlan 跳过，所以它永远进不了切片 ——
      // 与消息流里那张卡片显示"未生效"的结论一致。
      const plan =
        message.role === "assistant" ? collectCurrentPlan([message]) : null;
      // 收尾态的数据源：非空清单被记住（连同它自己声明的 done）；人消息一到就清掉。
      const rememberPlan =
        plan && plan.todos.length > 0
          ? { lastNonEmptyTodos: plan.todos, lastPlanDone: plan.done }
          : {};
      // 真人消息到达 = 用户开始下一件事 → **上一轮的计划整块退场**：pill 不该在
      // 新一轮里继续挂着上一轮（已经做完的）清单，模型下次更新会把它带回来。
      //
      // 用 `[]` 而不是 `null`：null 会命中 hydrate 两条路径的重建守卫
      // （`currentTodos === null` → 从消息窗口重新扫），把上一轮的计划又读回来。
      const isRealUserMessage =
        message.role === "user" && !message.autoGenerated;
      const clearRemembered = isRealUserMessage
        ? {
            currentTodos: [],
            currentPlanDone: false,
            lastNonEmptyTodos: null,
        ...
        : // 自动续跑（goal / 子代理完成通告）注入的提示也是 role: "user"，但它不是
          // 用户发的、更不是回合边界（同一约定见 utils/subagent-card.ts 的
          // collectCurrentRoundToolCallIds）——它还在一轮中间，**不能**把正在飞的
          // 计划抹掉；只退收尾态（与本次改动之前的行为一致）。
          message.role === "user"
          ? { lastNonEmptyTodos: null, lastPlanDone: false }
          : {};
      ...
      return {
        sessionStates: patchSession(state.sessionStates, sessionId, {
          ...
          ...(plan
            ? { currentTodos: plan.todos, currentPlanDone: plan.done }
            : {}),
          ...rememberPlan,
          ...clearRemembered,
          ...(shouldClearPartial
  ...
  setMessagesTail: (sessionId, messages, hasMore) =>
    set((state) => {
      ...
      const rebuilt = existing === null ? collectCurrentPlan(restored) : null;
      return {
        sessionStates: patchSession(state.sessionStates, sessionId, {
          messages: restored,
          ...
          ...(rebuilt
            ? { currentTodos: rebuilt.todos, currentPlanDone: rebuilt.done }
            : {}),
          // 首次水合重建出的就是非空清单 → 一并作为"最后一份非空"记住；
          // 重建出「已清空」时不写 —— 重启后不显示收尾态，也不显示「已完成」
          ...(rebuilt && rebuilt.todos.length > 0
            ? { lastNonEmptyTodos: rebuilt.todos, lastPlanDone: rebuilt.done }
            : {}),
        }),
      };
    }),
  ...
  prependOlderMessages: (sessionId, older, hasMore) => {
    ...
    set((state) => {
      ...
      const rebuiltFromPaging =
        ss.currentTodos === null ? collectCurrentPlan(merged) : null;
      let messages = merged;
      ...
      return {
        sessionStates: patchSession(state.sessionStates, sessionId, {
          ...
          ...(rebuiltFromPaging
            ? {
                currentTodos: rebuiltFromPaging.todos,
                currentPlanDone: rebuiltFromPaging.done,
              }
            : {}),
          ...(rebuiltFromPaging && rebuiltFromPaging.todos.length > 0
            ? {
                lastNonEmptyTodos: rebuiltFromPaging.todos,
                lastPlanDone: rebuiltFromPaging.done,
              }
            : {}),
        }),
      };
