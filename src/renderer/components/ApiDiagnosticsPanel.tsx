...
function PipelineNode({ step }: { step: DiagnosticStep }) {
  ...
  const label =
    t(`api.diagnostic.step.${step.name}`, "") ||
    STEP_NAME_FALLBACKS[step.name] ||
    step.name;
  ...
  return (
    <div className="flex flex-col items-center gap-1 min-w-0">
      ...
      {step.latencyMs !== undefined && step.status !== "pending" ? (
        ...
        <span className="text-xs text-transparent leading-tight select-none">
          -
        </span>
      )}
    </div>
...
const PENDING_STEP_NAMES: DiagnosticStepName[] = [
  "dns",
  "tcp",
  "tls",
  "auth",
        ...
        <div className="rounded-xl bg-background border border-border p-4">
          ...
          {result && !result.skippedReason && !isRunning && (
            <div className="mt-3 pt-3 border-t border-border flex items-center justify-between text-sm">
              ...
              >
                {result.overallOk
                  ? t("api.diagnostic.overallSuccess", {
                      ms: result.totalLatencyMs,
                    })
