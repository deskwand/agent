...
export function SettingsSandbox() {
  ...
  async function handleCheckStatus() {
    ...
    try {
      ...
      setError({
        text: err instanceof Error ? err.message : t("sandbox.checkFailed"),
      });
    } finally {
      setIsChecking(false);
    }
  ...
  async function handleInstallNode() {
    ...
    try {
      ...
      if (isWindows && status.wsl?.distro) {
        result = await window.electronAPI.sandbox.installNodeInWSL(
          status.wsl.distro,
        );
      } else if (isMac) {
      ...
      setError({
        text:
          err instanceof Error ? err.message : t("sandbox.nodeInstallFailed"),
      });
    } finally {
  ...
  async function handleInstallPython() {
    ...
    try {
      let result = false;
      if (isWindows && status.wsl?.distro) {
        result = await window.electronAPI.sandbox.installPythonInWSL(
          status.wsl.distro,
        );
      } else if (isMac) {
      ...
      setError({
        text:
          err instanceof Error ? err.message : t("sandbox.pythonInstallFailed"),
      });
    } finally {
      setIsInstalling(null);
    }
  ...
  async function handleRetrySetup() {
    ...
    try {
      ...
      setError({
        text: err instanceof Error ? err.message : t("sandbox.setupFailed"),
      });
    } finally {
      setIsInstalling(null);
    }
  ...
  async function handleStartLima() {
    ...
    try {
      ...
      setError({
        text: err instanceof Error ? err.message : t("sandbox.limaStartFailed"),
      });
    } finally {
      setIsInstalling(null);
    }
  ...
  async function handleStopLima() {
    ...
    try {
      ...
      setError({
        text: err instanceof Error ? err.message : t("sandbox.limaStopFailed"),
      });
    } finally {
      setIsInstalling(null);
    }
    ...
    <div className="space-y-4">
      ...
      {sandboxEnabled && sandboxAvailable && !sandboxReady && (
        <button
          ...
          className="w-full py-3 px-4 rounded-lg bg-accent text-accent-foreground font-medium hover:bg-accent-hover disabled:opacity-50 disabled:cursor-not-allowed transition-colors active:scale-[0.98] flex items-center justify-center gap-2"
        >
          {isInstalling === "setup" ? (
            <>
