...
export function SettingsLogs({ isActive }: { isActive: boolean }) {
  ...
  async function handleToggleDevLogs() {
    ...
    try {
      ...
      if (result.success) {
        ...
        setSuccess(
          newEnabled ? t("logs.devLogsEnabled") : t("logs.devLogsDisabled"),
        );
  ...
  return (
    <div className="space-y-4">
      ...
      <section className="rounded-2xl border border-border-subtle bg-background px-4 py-4">
      ...
      <SettingsContentSection
        title={t("logs.logFiles")}
        description={t("logs.recentDescription")}
      >
        {logFiles.length === 0 ? (
          ...
          <div className="space-y-2 max-h-64 overflow-y-auto">
            {logFiles.map((file) => (
              <div
                key={file.path}
                className="p-3 rounded-xl bg-background border border-border-subtle"
              >
      ...
      >
        <div className="grid grid-cols-3 gap-2">
          <button
            ...
            disabled={isLoading}
            className="py-3 px-4 rounded-lg bg-accent text-accent-foreground font-medium hover:bg-accent-hover disabled:opacity-50 disabled:cursor-not-allowed transition-colors active:scale-[0.98] flex items-center justify-center gap-2"
          >
            {isLoading ? (
