...
export function SettingsConnectors({ isActive }: { isActive: boolean }) {
  ...
  const loadServers = useCallback(async () => {
    try {
      const loaded =
        (await window.electronAPI.mcp.getServers()) as MCPServerConfig[];
      setServers(loaded || []);
      setError("");
    } catch (err) {
  ...
  const loadStatuses = useCallback(async () => {
    try {
      const loaded =
        (await window.electronAPI.mcp.getServerStatus()) as MCPServerStatus[];
      setStatuses(loaded || []);
    } catch (err) {
  ...
  async function handleDeleteServer(serverId: string) {
    ...
    try {
      ...
      setError(
        err instanceof Error ? err.message : t("mcp.deleteServerFailed"),
      );
    } finally {
      setIsLoading(false);
    }
...
function ServerCard({
  ...
  const serverStatus =
    status?.status ?? (server.enabled ? "connecting" : "disabled");
...
function ServerForm({
  ...
  const [type, setType] = useState<"stdio" | "sse" | "streamable-http">(
    server?.type || "stdio",
  );
    ...
    >
                  ...
                  <div className="space-y-2 p-2 rounded bg-background border border-accent/30">
                    ...
                    <div className="flex gap-2">
                      <button
                        ...
                        className="flex-1 py-1 px-3 rounded bg-accent text-accent-foreground text-xs hover:bg-accent-hover disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
                      >
                        {t("common.save")}
                      </button>
                      <button
      ...
      <div className="flex gap-2">
        <button
          type="submit"
          disabled={isLoading}
          className="flex-1 py-2 px-4 rounded-lg bg-accent text-accent-foreground hover:bg-accent-hover disabled:opacity-50 transition-colors flex items-center justify-center gap-2"
        >
          {isLoading ? (
            <>
