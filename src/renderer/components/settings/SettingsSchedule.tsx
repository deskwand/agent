...
export function SettingsSchedule({ isActive }: { isActive: boolean }) {
  ...
  async function toggleTask(task: ScheduleTask) {
    ...
    setSuccess(null);
    try {
      const updated = await window.electronAPI.schedule.toggle(
        task.id,
        !task.enabled,
      );
      if (!updated) {
      ...
      setSuccess({
        key: updated.enabled ? "schedule.taskEnabled" : "schedule.taskDisabled",
      });
      await loadTasks();
    } catch (err) {
      setError(
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="rounded-2xl border border-border bg-surface p-4 space-y-3 shadow-card">
        <h4 className="text-sm font-medium text-text-primary">
          {editingId ? t("schedule.editTitle") : t("schedule.createTitle")}
        </h4>
        <div className="rounded-lg border border-border bg-background px-3 py-2">
          <div className="text-xs text-text-muted mb-1">
            {t("schedule.autoTitleLabel")}
          </div>
          ...
          {scheduleMode === "legacy-interval" && (
            <div className="grid grid-cols-2 gap-2">
              ...
              <select
                value={repeatUnit}
                onChange={(e) =>
                  setRepeatUnit(e.target.value as ScheduleRepeatUnit)
                }
                className="w-full px-3 py-2 rounded-lg bg-surface border border-border text-sm"
              >
        ...
        <div className="flex items-center gap-2">
          <button
            onClick={submitTask}
            disabled={isLoading}
            className="px-3 py-2 rounded-lg bg-accent text-accent-foreground text-sm disabled:opacity-50"
          >
      ...
      <div className="space-y-2">
        ...
        {tasks.length === 0 ? (
          ...
          tasks.map((task) => (
            <div
              key={task.id}
              className="rounded-2xl border border-border bg-surface p-3 space-y-2 shadow-card"
            >
              {(() => {
                const lastRunSession = task.lastRunSessionId
                  ? (sessions.find(
                      (session) => session.id === task.lastRunSessionId,
                    ) ?? null)
                  : null;
                ...
                return (
                  <>
                    <div className="flex items-center justify-between gap-2">
                      ...
                      >
                        {task.enabled
                          ? t("schedule.taskEnabled")
                          : t("schedule.taskDisabled")}
                      </span>
                    </div>
                    <div className="text-xs text-text-muted">
                      {task.nextRunAt === null
                    ...
                    {task.lastRunSessionId && (
                      <div className="text-xs text-text-muted break-all">
                        {t("schedule.recentSession", {
                          value: task.lastRunSessionId,
                        })}
                      </div>
                    )}
                    <div className="text-xs text-text-muted">
                      {t("schedule.sessionStatus", {
                        value: lastRunStatusLabel,
                      })}
                    </div>
                    <div
...
function buildScheduleConfigFromForm(
  ...
  if (mode === "weekly" && normalizedTimes.length > 0 && weekdays.length > 0) {
    return {
      kind: "weekly",
      weekdays: Array.from(new Set(weekdays)).sort(
...
function TimeMultiSelectMenu(props: {
  ...
  const containerRef = useRef<HTMLDivElement | null>(null);
  const buttonText =
    summary || (values.length > 0 ? joinAppList(values) : placeholder);
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between gap-2">
                <div>
                  ...
                  <div className="text-xs text-text-muted">
                    {t("schedule.pickerAnyHHmm")}
                  </div>
                </div>
                {values.length > 0 && (
                  <div className="text-xs text-text-muted">
                    {t("schedule.pickerSelectedCount", {
                      count: values.length,
                    })}
                  </div>
                )}
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  ...
                  className="inline-flex min-w-[92px] flex-shrink-0 items-center justify-center gap-1 whitespace-nowrap rounded-lg bg-accent px-3 py-2 text-sm text-accent-foreground disabled:opacity-50"
                >
