...
function cloneRuntimeConfig(
  ...
  return {
    ...
    ingestionConcurrency:
      source.ingestionConcurrency ??
      DEFAULT_MEMORY_RUNTIME.ingestionConcurrency,
    ...
    evalArtifactsRoot:
      source.evalArtifactsRoot ?? DEFAULT_MEMORY_RUNTIME.evalArtifactsRoot,
    promptIterationRounds:
      source.promptIterationRounds ??
      DEFAULT_MEMORY_RUNTIME.promptIterationRounds,
  };
}
...
export function SettingsMemory() {
  ...
  const refreshFiles = async () => {
    ...
    if (
      selectedFilePath &&
      nextFiles.some((item) => item.filePath === selectedFilePath)
    ) {
      const nextContent =
        await window.electronAPI.memory.readFile(selectedFilePath);
  ...
  const handleInspectSession = async (
    ...
    setStatus(null);
    try {
      const detail = await window.electronAPI.memory.inspectSession(
        sessionId,
        workspaceKey,
      );
  ...
  return (
    <div className="space-y-6">
      ...
      >
        <div className="flex flex-col gap-3 rounded-2xl border border-border-muted bg-background-secondary/60 p-4">
          ...
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            <MetricCard
              label={t("memory.coreCount")}
              value={overview?.coreCount ?? 0}
            />
            <MetricCard
            ...
            <InfoCard
              ...
              value={
                overview?.failedSessionCount
                  ? t("memory.failedSessions", {
                      count: overview.failedSessionCount,
                    })
                  : t("memory.healthy")
              }
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            <InfoCard
              ...
              value={
                overview?.storageRoot ||
                runtimeDraft.storageRoot ||
                "Default userData/memory"
              }
            />
            <InfoCard
              ...
              secondary={
                overview?.topSourceWorkspaces?.length
                  ? `Top sources: ${overview.topSourceWorkspaces
                      ...
                      .map(
                        (item) =>
                          `${item.workspaceKey} (${item.sessionCount}/${item.chunkCount})`,
                      )
                      .join(" · ")}`
                  : undefined
              }
      ...
      >
        <div className="space-y-4 rounded-2xl border border-border-muted bg-background-secondary/60 p-4">
          <div className="grid gap-4 md:grid-cols-2">
            ...
            <LabeledField
              label={t("memory.ingestionConcurrency", "重建并发度")}
            >
              <input
                type="number"
                min={1}
          ...
          <div className="grid gap-4 md:grid-cols-3">
            <LabeledField label={t("memory.evalArtifactsRoot", "评测产物目录")}>
              <input
                ...
                onChange={(event) =>
                  setRuntimeDraft((prev) => ({
                    ...prev,
                    evalArtifactsRoot: event.target.value,
                  }))
                }
                className="w-full rounded-lg border border-border bg-background px-3 py-2.5 text-sm text-text-primary outline-none focus:border-accent"
              />
            ...
            <LabeledField
              label={t("memory.promptIterationRounds", "Prompt 迭代轮数")}
            >
              <input
                type="number"
                min={0}
          ...
          <div className="flex justify-end">
            <button
              ...
              disabled={isBusy}
              className="rounded-lg bg-accent px-4 py-2.5 text-sm font-medium text-accent-foreground disabled:cursor-not-allowed disabled:opacity-60"
            >
      ...
      >
        <div className="space-y-3 rounded-2xl border border-border-muted bg-background-secondary/60 p-4">
          <div className="flex flex-col gap-3 sm:flex-row">
            ...
            >
              {hasWorkspace && (
                <option value="workspace">{t("memory.scopeWorkspace")}</option>
              )}
                ...
                {inspectedSession ? (
                  <div className="mt-3 space-y-3">
                    ...
                    <pre className="max-h-48 overflow-auto rounded-lg bg-background-secondary/80 p-3 text-xs leading-5 text-text-secondary whitespace-pre-wrap">
                      {JSON.stringify(
                        inspectedSession.session.rawSession,
                        null,
                        2,
                      )}
                    </pre>
      ...
      >
        <div className="grid gap-4 rounded-2xl border border-border-muted bg-background-secondary/60 p-4 lg:grid-cols-[minmax(0,280px)_minmax(0,1fr)]">
          <div className="space-y-2">
...
function InfoCard({
  ...
  return (
    <div className="rounded-xl border border-border-muted bg-background/80 p-3 text-xs text-text-muted">
      ...
      {secondary ? (
        <p className="mt-2 break-all text-rose-500">{secondary}</p>
      ) : null}
    </div>
  );
}
