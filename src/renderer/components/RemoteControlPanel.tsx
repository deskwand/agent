...
export function RemoteControlPanel({ isActive }: { isActive: boolean }) {
    ...
    try {
      ...
      if (configResult) {
        ...
        setDefaultWorkingDirectory(
          configResult.gateway?.defaultWorkingDirectory || "",
        );
        setAutoApproveSafeTools(
          configResult.gateway?.autoApproveSafeTools !== false,
        );
        ...
        if (configResult.channels?.feishu) {
          ...
          setFeishuDmPolicy(
            configResult.channels.feishu.dm?.policy || "pairing",
          );
          setUseLongConnection(
            configResult.channels.feishu.useWebSocket !== false,
          );
        }
      }
    } catch (err) {
  ...
  async function approvePairing(request: PairingRequest) {
    if (!isElectron) return;
    try {
      await window.electronAPI.remote.approvePairing(
        request.channelType,
        request.userId,
      );
  ...
  async function revokePairing(user: PairedUser) {
    if (!isElectron) return;
    try {
      await window.electronAPI.remote.revokePairing(
        user.channelType,
        user.userId,
      );
      ...
      <div className="p-6 rounded-[2rem] border border-border-subtle bg-background/60">
        ...
        <div className="flex justify-end mt-6 pt-6 border-t border-border">
          <button
            onClick={saveConfig}
            disabled={isSaving}
            className="flex items-center gap-2 px-6 py-2.5 bg-accent hover:bg-accent/90 text-accent-foreground rounded-xl font-medium transition-colors disabled:opacity-50"
          >
            {isSaving ? (
