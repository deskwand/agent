...
export interface Message {
  ...
  code?: string; // 结构化错误码（如 INSUFFICIENT_BALANCE），可选、向后兼容
  localStatus?: "queued" | "cancelled";
  executionTimeMs?: number;
  autoGenerated?: boolean;
  /**
   * 用户消息引用的页面元素（Design Mode）。**只用于气泡回显**：
   * 模型看到的上下文是 SDK 的隐藏 custom message，不是这个字段。
   */
...
export interface TextContent {
  ...
  text: string;
  /**
   * 由宿主合成的上下文块（如浏览器元素拾取），不是用户敲的字。
   * 仅供宿主内存 content 通道和显示过滤使用；SDK 持久化另走隐藏 custom message。
   * 显示层必须过滤掉它，否则用户气泡里会出现一坨 <selected-element> XML。
   */
  synthetic?: boolean;
}
...
export interface QueuedInput {
  ...
  /**
   * 元素磁贴快照。必须随队列条目保下来 —— 运行中提交时「立即引导」
   * 是按纯文本发出去的，不存快照就会静默丢掉元素附件。
   */
