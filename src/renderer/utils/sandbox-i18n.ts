...
function translateSetupDetail(
  ...
  if (nodeWslMatch) {
    return t("sandbox.setupDetailInstallNodeWslRuntime", {
      distro: nodeWslMatch[1],
    });
  }
  const pythonWslMatch = detail.match(/^Installing Python runtime in (.+)$/);
  if (pythonWslMatch) {
    return t("sandbox.setupDetailInstallPythonWslRuntime", {
      distro: pythonWslMatch[1],
    });
  }
  const pipWslMatch = detail.match(
    /^Installing Python package manager in (.+)$/,
  );
  if (pipWslMatch) {
    return t("sandbox.setupDetailInstallPipWslRuntime", {
      distro: pipWslMatch[1],
    });
  }
  const map: Record<string, string> = {
    ...
    "Install WSL2 for better sandbox isolation":
      "sandbox.setupDetailInstallWsl",
    "Please install Node.js manually in WSL":
      "sandbox.setupDetailInstallNodeWsl",
    ...
    "First run requires image download, may take a few minutes":
      "sandbox.setupDetailCreateLima",
    "VM startup may take a few minutes": "sandbox.setupDetailStartLima",
    "Installing Node.js runtime in Lima VM":
      "sandbox.setupDetailInstallNodeLima",
    "Installing Python runtime in Lima VM":
      "sandbox.setupDetailInstallPythonLima",
  };
...
function translateSyncDetail(
  t: TFunction,
  detail?: string,
): string | undefined {
  if (!detail) return undefined;
  const map: Record<string, string> = {
    "Copying project files to isolated WSL environment":
      "sandbox.syncDetailWsl",
    "Copying project files to isolated Lima environment":
      "sandbox.syncDetailLima",
    "Copying built-in skills to sandbox": "sandbox.syncDetailSkills",
    "Falling back to direct access mode (less secure)":
      "sandbox.syncDetailFallback",
  };
