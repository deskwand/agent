...
export function truncateRendererLogText(
  value: string,
  maxLength = MAX_RENDERER_LOG_TEXT,
): string {
...
export function normalizeRendererLogValue(
  ...
  if (value instanceof Event) {
    ...
    return {
      ...
      targetId:
        target && "id" in target
          ? (target as HTMLElement).id || undefined
          : undefined,
    };
  }
  ...
  if (typeof value === "object") {
    ...
    const limitedEntries = entries
      ...
      .map(([key, item]) => [
        key,
        normalizeRendererLogValue(item, seen, depth + 1),
      ]);
...
export class RendererDiagnosticsDeduper {
  ...
  shouldReport(args: unknown[], now = Date.now()): boolean {
    ...
    const fingerprint = JSON.stringify(
      args.map((arg) => normalizeRendererLogValue(arg)),
    );
