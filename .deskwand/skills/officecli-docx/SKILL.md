...
**officecli ships with DeskWand and is already on your PATH. Never install it.**

```bash
officecli --version    # must print a version; run this first if unsure
```

If it prints `command not found`, the app's bundled binaries are missing or
misconfigured. Do **not** fetch or run any installer — report the problem to
the user and stop. Installing a second copy would shadow the pinned, tested
build and break the app's offline guarantee.
