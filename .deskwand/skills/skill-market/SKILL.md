...
- **安装即生效**：deskwand agent-runner 检测 skillsSignature 变化后，下一条消息自动重建 pi session，无需手动重启
...
1. `cat ~/.deskwand/skills/{name}/SKILL.md | head -5` — 确认文件完整可读
2. 检查 frontmatter 有 `name` 和 `version` 字段
3. 若 requires 声明了 bins：逐一 `which {bin}` 检查
4. 若 requires 声明了 skills：检查本地 `~/.deskwand/skills/{dep}/` 存在，缺则递归安装（上限 3 层）
